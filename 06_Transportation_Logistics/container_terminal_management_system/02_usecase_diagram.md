# Use Case Diagram — Container Terminal Management System
...
flowchart LR
    ...
    subgraph SystemBoundary["Container Terminal Management System"]
...
|---|-------|------------|------------------|-------------------|
| 1 | Terminal Operations Manager | Primary | Initiates operational requests, monitors real-time progress, manages bookings/orders | UC01, UC02, UC03, UC04 |
| 2 | Crane / Equipment Operator | Primary | Receives dispatch assignments, executes physical operations, reports progress and issues | UC05, UC06, UC07, UC08 |
| 3 | Vessel Agent | Primary | Oversees route planning, resource allocation, real-time tracking, and analytics | UC09, UC10, UC11, UC14 |
| 4 | Port Administrator | Primary | Configures system parameters, manages asset registries, provisions user roles | UC12, UC13 |

## Use Case Table | Bảng Use Case

| # | UC ID | Use Case Name | Primary Actor | Secondary Actor | Description | Priority |
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Operational Request | Terminal Operations Manager | System | Creates a new transport, dispatch, or operational order | High |
| 2 | UC02 | Track Execution Status | Terminal Operations Manager | Telematics Gateway | Provides real-time visibility and status tracking for active requests | High |
| 3 | UC03 | Cancel or Modify Request | Terminal Operations Manager | Dispatcher | Allows updating or cancelling pending requests prior to execution | Medium |
| 4 | UC04 | Authenticate User | System | Auth Gateway | Validates user identity, credentials, and role permissions | High |
| 5 | UC05 | Accept & Execute Task | Crane / Equipment Operator | System | Acknowledges assigned operational task and marks start of execution | High |
| 6 | UC06 | Update Task Progress | Crane / Equipment Operator | Telematics Gateway | Records status milestones, timestamps, and GPS checkpoints | High |
| 7 | UC07 | Log Incident or Delay | Crane / Equipment Operator | Dispatcher | Reports traffic congestion, vehicle breakdown, or cargo exceptions | Medium |
| 8 | UC08 | Complete Task Execution | Crane / Equipment Operator | Customer | Finalizes task, captures proof of delivery/execution, closes assignment | High |
| 9 | UC09 | Optimize Route & Schedule | Vessel Agent | Map API | Calculates optimal paths, schedules, and load distributions | High |
| 10 | UC10 | Dispatch Resource & Assignment | Vessel Agent | Field Agent | Assigns vehicles, drivers, or equipment to scheduled tasks | High |
| 11 | UC11 | Monitor Real-time Dashboard | Vessel Agent | Telematics Gateway | Displays live fleet/asset map, active jobs, and SLA alerts | High |
| 12 | UC12 | Manage Asset & Master Data | Port Administrator | System | Registers vehicles, drivers, locations, and rate cards | Medium |
| 13 | UC13 | Configure System Rules | Port Administrator | System | Sets dispatch algorithms, notification triggers, and threshold limits | Medium |
| 14 | UC14 | Generate Performance Reports | Vessel Agent | Audit System | Compiles operational efficiency, fuel, SLA compliance, and cost analytics | Medium |
...
| **Actor(s)** | Primary: Terminal Operations Manager <br> Secondary: System |
...
| **Actor(s)** | Primary: Crane / Equipment Operator |
...
| **Actor(s)** | Primary: Crane / Equipment Operator <br> Secondary: Telematics Gateway |
...
| **Actor(s)** | Primary: Crane / Equipment Operator <br> Secondary: Customer |
...
| **Actor(s)** | Primary: Vessel Agent <br> Secondary: Map API |
