# Use Case Diagram — Electric Scooter Sharing System

## Mermaid Code

```mermaid
flowchart LR
    actor_P1["Rider / Commuter"]
    actor_P2["Field Maintenance Tech"]
    actor_P3["City Operations Lead"]
    actor_P4["System Administrator"]

    subgraph SystemBoundary["Electric Scooter Sharing System"]
...
|---|-------|------------|------------------|-------------------|
| 1 | Rider / Commuter | Primary | Initiates operational requests, monitors real-time progress, manages bookings/orders | UC01, UC02, UC03, UC04 |
| 2 | Field Maintenance Tech | Primary | Receives dispatch assignments, executes physical operations, reports progress and issues | UC05, UC06, UC07, UC08 |
| 3 | City Operations Lead | Primary | Oversees route planning, resource allocation, real-time tracking, and analytics | UC09, UC10, UC11, UC14 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Operational Request | Rider / Commuter | System | Creates a new transport, dispatch, or operational order | High |
| 2 | UC02 | Track Execution Status | Rider / Commuter | Telematics Gateway | Provides real-time visibility and status tracking for active requests | High |
| 3 | UC03 | Cancel or Modify Request | Rider / Commuter | Dispatcher | Allows updating or cancelling pending requests prior to execution | Medium |
| 4 | UC04 | Authenticate User | System | Auth Gateway | Validates user identity, credentials, and role permissions | High |
| 5 | UC05 | Accept & Execute Task | Field Maintenance Tech | System | Acknowledges assigned operational task and marks start of execution | High |
| 6 | UC06 | Update Task Progress | Field Maintenance Tech | Telematics Gateway | Records status milestones, timestamps, and GPS checkpoints | High |
| 7 | UC07 | Log Incident or Delay | Field Maintenance Tech | Dispatcher | Reports traffic congestion, vehicle breakdown, or cargo exceptions | Medium |
| 8 | UC08 | Complete Task Execution | Field Maintenance Tech | Customer | Finalizes task, captures proof of delivery/execution, closes assignment | High |
| 9 | UC09 | Optimize Route & Schedule | City Operations Lead | Map API | Calculates optimal paths, schedules, and load distributions | High |
| 10 | UC10 | Dispatch Resource & Assignment | City Operations Lead | Field Agent | Assigns vehicles, drivers, or equipment to scheduled tasks | High |
| 11 | UC11 | Monitor Real-time Dashboard | City Operations Lead | Telematics Gateway | Displays live fleet/asset map, active jobs, and SLA alerts | High |
...
| 14 | UC14 | Generate Performance Reports | City Operations Lead | Audit System | Compiles operational efficiency, fuel, SLA compliance, and cost analytics | Medium |
...
| **Actor(s)** | Primary: Rider / Commuter <br> Secondary: System |
...
| **Actor(s)** | Primary: Field Maintenance Tech |
...
| **Actor(s)** | Primary: Field Maintenance Tech <br> Secondary: Telematics Gateway |
...
| **Actor(s)** | Primary: Field Maintenance Tech <br> Secondary: Customer |
...
| **Actor(s)** | Primary: City Operations Lead <br> Secondary: Map API |
