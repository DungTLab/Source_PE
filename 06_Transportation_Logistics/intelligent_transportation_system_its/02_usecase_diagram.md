# Use Case Diagram — Intelligent Transportation System (ITS)

## Mermaid Code

```mermaid
flowchart LR
    actor_P1["Vehicle Owner / Driver"]
    actor_P2["Traffic Operations Officer"]
    actor_P3["Enforcement Agent"]
    actor_P4["System Administrator"]

    subgraph SystemBoundary["Intelligent Transportation System (ITS)"]
...
|---|-------|------------|------------------|-------------------|
| 1 | Vehicle Owner / Driver | Primary | Initiates operational requests, monitors real-time progress, manages bookings/orders | UC01, UC02, UC03, UC04 |
| 2 | Traffic Operations Officer | Primary | Receives dispatch assignments, executes physical operations, reports progress and issues | UC05, UC06, UC07, UC08 |
| 3 | Enforcement Agent | Primary | Oversees route planning, resource allocation, real-time tracking, and analytics | UC09, UC10, UC11, UC14 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Operational Request | Vehicle Owner / Driver | System | Creates a new transport, dispatch, or operational order | High |
| 2 | UC02 | Track Execution Status | Vehicle Owner / Driver | Telematics Gateway | Provides real-time visibility and status tracking for active requests | High |
| 3 | UC03 | Cancel or Modify Request | Vehicle Owner / Driver | Dispatcher | Allows updating or cancelling pending requests prior to execution | Medium |
| 4 | UC04 | Authenticate User | System | Auth Gateway | Validates user identity, credentials, and role permissions | High |
| 5 | UC05 | Accept & Execute Task | Traffic Operations Officer | System | Acknowledges assigned operational task and marks start of execution | High |
| 6 | UC06 | Update Task Progress | Traffic Operations Officer | Telematics Gateway | Records status milestones, timestamps, and GPS checkpoints | High |
| 7 | UC07 | Log Incident or Delay | Traffic Operations Officer | Dispatcher | Reports traffic congestion, vehicle breakdown, or cargo exceptions | Medium |
| 8 | UC08 | Complete Task Execution | Traffic Operations Officer | Customer | Finalizes task, captures proof of delivery/execution, closes assignment | High |
| 9 | UC09 | Optimize Route & Schedule | Enforcement Agent | Map API | Calculates optimal paths, schedules, and load distributions | High |
| 10 | UC10 | Dispatch Resource & Assignment | Enforcement Agent | Field Agent | Assigns vehicles, drivers, or equipment to scheduled tasks | High |
| 11 | UC11 | Monitor Real-time Dashboard | Enforcement Agent | Telematics Gateway | Displays live fleet/asset map, active jobs, and SLA alerts | High |
...
| 14 | UC14 | Generate Performance Reports | Enforcement Agent | Audit System | Compiles operational efficiency, fuel, SLA compliance, and cost analytics | Medium |
...
| **Actor(s)** | Primary: Vehicle Owner / Driver <br> Secondary: System |
...
| **Actor(s)** | Primary: Traffic Operations Officer |
...
| **Actor(s)** | Primary: Traffic Operations Officer <br> Secondary: Telematics Gateway |
...
| **Actor(s)** | Primary: Traffic Operations Officer <br> Secondary: Customer |
...
| **Actor(s)** | Primary: Enforcement Agent <br> Secondary: Map API |
