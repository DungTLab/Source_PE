# Use Case Diagram — Rail Freight Management System

## Mermaid Code

```mermaid
flowchart LR
    actor_P1["Commuter / Passenger"]
    actor_P2["Bus / Train Operator"]
    actor_P3["Transit Dispatcher"]
    actor_P4["Transit Authority Manager"]

    subgraph SystemBoundary["Rail Freight Management System"]
...
|---|-------|------------|------------------|-------------------|
| 1 | Commuter / Passenger | Primary | Initiates operational requests, monitors real-time progress, manages bookings/orders | UC01, UC02, UC03, UC04 |
| 2 | Bus / Train Operator | Primary | Receives dispatch assignments, executes physical operations, reports progress and issues | UC05, UC06, UC07, UC08 |
| 3 | Transit Dispatcher | Primary | Oversees route planning, resource allocation, real-time tracking, and analytics | UC09, UC10, UC11, UC14 |
| 4 | Transit Authority Manager | Primary | Configures system parameters, manages asset registries, provisions user roles | UC12, UC13 |

## Use Case Table | Bảng Use Case

| # | UC ID | Use Case Name | Primary Actor | Secondary Actor | Description | Priority |
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Operational Request | Commuter / Passenger | System | Creates a new transport, dispatch, or operational order | High |
| 2 | UC02 | Track Execution Status | Commuter / Passenger | Telematics Gateway | Provides real-time visibility and status tracking for active requests | High |
| 3 | UC03 | Cancel or Modify Request | Commuter / Passenger | Dispatcher | Allows updating or cancelling pending requests prior to execution | Medium |
| 4 | UC04 | Authenticate User | System | Auth Gateway | Validates user identity, credentials, and role permissions | High |
| 5 | UC05 | Accept & Execute Task | Bus / Train Operator | System | Acknowledges assigned operational task and marks start of execution | High |
| 6 | UC06 | Update Task Progress | Bus / Train Operator | Telematics Gateway | Records status milestones, timestamps, and GPS checkpoints | High |
| 7 | UC07 | Log Incident or Delay | Bus / Train Operator | Dispatcher | Reports traffic congestion, vehicle breakdown, or cargo exceptions | Medium |
| 8 | UC08 | Complete Task Execution | Bus / Train Operator | Customer | Finalizes task, captures proof of delivery/execution, closes assignment | High |
| 9 | UC09 | Optimize Route & Schedule | Transit Dispatcher | Map API | Calculates optimal paths, schedules, and load distributions | High |
| 10 | UC10 | Dispatch Resource & Assignment | Transit Dispatcher | Field Agent | Assigns vehicles, drivers, or equipment to scheduled tasks | High |
| 11 | UC11 | Monitor Real-time Dashboard | Transit Dispatcher | Telematics Gateway | Displays live fleet/asset map, active jobs, and SLA alerts | High |
| 12 | UC12 | Manage Asset & Master Data | Transit Authority Manager | System | Registers vehicles, drivers, locations, and rate cards | Medium |
| 13 | UC13 | Configure System Rules | Transit Authority Manager | System | Sets dispatch algorithms, notification triggers, and threshold limits | Medium |
| 14 | UC14 | Generate Performance Reports | Transit Dispatcher | Audit System | Compiles operational efficiency, fuel, SLA compliance, and cost analytics | Medium |
...
| **Actor(s)** | Primary: Commuter / Passenger <br> Secondary: System |
...
| **Actor(s)** | Primary: Bus / Train Operator |
...
| **Actor(s)** | Primary: Bus / Train Operator <br> Secondary: Telematics Gateway |
...
| **Actor(s)** | Primary: Bus / Train Operator <br> Secondary: Customer |
...
| **Actor(s)** | Primary: Transit Dispatcher <br> Secondary: Map API |
