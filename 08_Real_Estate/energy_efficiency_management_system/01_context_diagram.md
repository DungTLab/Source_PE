# Context Diagram — Energy Efficiency Management System (Buildings)

## Mermaid Code

```mermaid
flowchart LR
    Actor1["Resident / Occupant"]
    Actor2["Facility Manager"]
    Actor3["Maintenance Engineer"]
    Actor4["Board Committee Member"]
    ...
    System(("System Name: Energy Efficiency Management System (Buildings)"))
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Resident / Occupant | Primary | service request, input parameters, verification documents | status confirmation, processing results, system alerts | Primary end user interacting with core workflows. |
| 2 | Facility Manager | Primary | operation logs, updated parameters, execution status | assigned tasks, client requests, performance reports | Operational role managing tasks and service requests. |
| 3 | Maintenance Engineer | Primary | approval decision, configuration parameters, override commands | approval queue, system metrics, compliance reports | Administrative user overseeing compliance and settings. |
| 4 | Board Committee Member | Primary | audit query, report parameters, policy update | audit logs, aggregate metrics, anomaly alerts | Supervisory role checking quality control. |
