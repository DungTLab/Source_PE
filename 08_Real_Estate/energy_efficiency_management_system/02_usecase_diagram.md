# Use Case Diagram — Energy Efficiency Management System (Buildings)

## Mermaid Code

```mermaid
flowchart LR
    actor_1["Resident / Occupant"]
    actor_2["Facility Manager"]
    actor_3["Maintenance Engineer"]
    actor_4["Board Committee Member"]
    ...
    subgraph SystemBoundary["Energy Efficiency Management System (Buildings)"]
        UC01(["Submit Energy Efficiency Management (Buildings) Request"])
        UC02(["View Energy Efficiency Management (Buildings) Dashboard"])
        UC03(["Manage Profile & Settings"])
        UC04(["Process Energy Efficiency Management (Buildings) Application"])
...
|---|-------|------------|------------------|-------------------|
| 1 | Resident / Occupant | Primary | Main user initiating energy efficiency management system (buildings) operations. | UC01, UC02, UC03, UC11, UC12 |
| 2 | Facility Manager | Primary | Handles operational workflows in energy efficiency management system (buildings). | UC04, UC05, UC06, UC13 |
| 3 | Maintenance Engineer | Primary | Manages system configurations and governance. | UC07, UC08, UC10, UC14 |
| 4 | Board Committee Member | Primary | Monitors platform integrity and audit compliance. | UC09 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Energy Efficiency Management (Buildings) Request | Resident / Occupant | Notification & SMS Gateway | Initiate a new request or operation within Energy Efficiency Management System (Buildings). | High |
| 2 | UC02 | View Energy Efficiency Management (Buildings) Dashboard | Resident / Occupant | None | Display overview summary metrics, pending actions, and status updates. | High |
| 3 | UC03 | Manage Profile & Settings | Resident / Occupant | None | Update account information, notification settings, and preferences. | Medium |
| 4 | UC04 | Process Energy Efficiency Management (Buildings) Application | Facility Manager | Document Storage & e-Sign API | Evaluate submitted data, verify documents, and perform operational tasks. | High |
| 5 | UC05 | Update Status & Workflow State | Facility Manager | Notification & SMS Gateway | Advance record through lifecycle states (Pending, Active, Closed). | High |
| 6 | UC06 | Generate Operational Reports | Facility Manager | None | Extract detailed summary statistics, operational metrics, and logs. | Medium |
| 7 | UC07 | Approve Operational Submission | Maintenance Engineer | Government Regulatory Portal | Review and approve/reject high-level operational or legal requests. | High |
| 8 | UC08 | Configure System Parameters | Maintenance Engineer | None | Set business rules, approval thresholds, and automated triggers. | High |
| 9 | UC09 | Perform Compliance Audit | Board Committee Member | Government Regulatory Portal | Inspect audit trails and ensure regulatory compliance. | Medium |
| 10 | UC10 | Manage System User Roles | Maintenance Engineer | None | Assign access permissions, roles, and security policies. | High |
| 11 | UC11 | Process Payment Transaction | Payment Gateway API | Resident / Occupant | Execute billing charges, fee calculations, and electronic payouts. | High |
| 12 | UC12 | Dispatch Automated Alerts | Notification & SMS Gateway | Resident / Occupant | Trigger automated SMS, push notifications, and email notices. | Medium |
| 13 | UC13 | Store & Verify Digital Documents | Document Storage & e-Sign API | Facility Manager | Archive PDF contracts, proof documents, and compute hash verification. | High |
| 14 | UC14 | Verify Regulatory Compliance | Government Regulatory Portal | Maintenance Engineer | Query government databases for license checks and regulatory approvals. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Submit Energy Efficiency Management (Buildings) Request |
| **Actor(s)** | Primary: Resident / Occupant / Secondary: Notification & SMS Gateway |
| **Description** | Allows Resident / Occupant to submit a formal request or transaction within Energy Efficiency Management System (Buildings) with validated inputs. |
| **Precondition** | 1. User is authenticated and logged into the system.<br>2. System services are active and accessible. |
| **Main Flow** | 1. User accesses the request submission page.<br>2. System presents input form tailored for Energy Efficiency Management System (Buildings).<br>3. User enters required details, uploads supporting files.<br>4. System validates inputs against business rules and data schemas.<br>5. User reviews summary and confirms submission.<br>6. System stores record with status 'PENDING_REVIEW'.<br>7. System triggers Notification & SMS Gateway to send confirmation notice to User. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Process Energy Efficiency Management (Buildings) Application |
| **Actor(s)** | Primary: Facility Manager / Secondary: Document Storage & e-Sign API |
| **Description** | Allows Facility Manager to review, inspect, and process user requests. |
...
| **Actor(s)** | Primary: Maintenance Engineer / Secondary: Government Regulatory Portal |
| **Description** | Enables Maintenance Engineer to issue final executive approval or authorization for high-priority items. |
...
| **Actor(s)** | Primary: Payment Gateway API / Secondary: Resident / Occupant |
