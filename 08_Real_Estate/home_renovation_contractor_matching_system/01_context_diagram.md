# Context Diagram — Home Renovation & Contractor Matching System

## Mermaid Code

```mermaid
flowchart LR
    Actor1["Platform User / Client"]
    Actor2["Service Provider / Agent"]
    Actor3["System Administrator"]
    Actor4["Platform Auditor"]
    ...
    System(("System Name: Home Renovation & Contractor Matching System"))
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Platform User / Client | Primary | service request, input parameters, verification documents | status confirmation, processing results, system alerts | Primary end user interacting with core workflows. |
| 2 | Service Provider / Agent | Primary | operation logs, updated parameters, execution status | assigned tasks, client requests, performance reports | Operational role managing tasks and service requests. |
| 3 | System Administrator | Primary | approval decision, configuration parameters, override commands | approval queue, system metrics, compliance reports | Administrative user overseeing compliance and settings. |
| 4 | Platform Auditor | Primary | audit query, report parameters, policy update | audit logs, aggregate metrics, anomaly alerts | Supervisory role checking quality control. |
