# Use Case Diagram — Home Renovation & Contractor Matching System

## Mermaid Code

```mermaid
flowchart LR
    actor_1["Platform User / Client"]
    actor_2["Service Provider / Agent"]
    actor_3["System Administrator"]
    actor_4["Platform Auditor"]
    ...
    subgraph SystemBoundary["Home Renovation & Contractor Matching System"]
        UC01(["Submit Home Renovation & Contractor Matching Request"])
        UC02(["View Home Renovation & Contractor Matching Dashboard"])
        UC03(["Manage Profile & Settings"])
        UC04(["Process Home Renovation & Contractor Matching Application"])
...
|---|-------|------------|------------------|-------------------|
| 1 | Platform User / Client | Primary | Main user initiating home renovation & contractor matching system operations. | UC01, UC02, UC03, UC11, UC12 |
| 2 | Service Provider / Agent | Primary | Handles operational workflows in home renovation & contractor matching system. | UC04, UC05, UC06, UC13 |
| 3 | System Administrator | Primary | Manages system configurations and governance. | UC07, UC08, UC10, UC14 |
| 4 | Platform Auditor | Primary | Monitors platform integrity and audit compliance. | UC09 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Home Renovation & Contractor Matching Request | Platform User / Client | Notification & SMS Gateway | Initiate a new request or operation within Home Renovation & Contractor Matching System. | High |
| 2 | UC02 | View Home Renovation & Contractor Matching Dashboard | Platform User / Client | None | Display overview summary metrics, pending actions, and status updates. | High |
| 3 | UC03 | Manage Profile & Settings | Platform User / Client | None | Update account information, notification settings, and preferences. | Medium |
| 4 | UC04 | Process Home Renovation & Contractor Matching Application | Service Provider / Agent | Document Storage & e-Sign API | Evaluate submitted data, verify documents, and perform operational tasks. | High |
| 5 | UC05 | Update Status & Workflow State | Service Provider / Agent | Notification & SMS Gateway | Advance record through lifecycle states (Pending, Active, Closed). | High |
| 6 | UC06 | Generate Operational Reports | Service Provider / Agent | None | Extract detailed summary statistics, operational metrics, and logs. | Medium |
| 7 | UC07 | Approve Operational Submission | System Administrator | Government Regulatory Portal | Review and approve/reject high-level operational or legal requests. | High |
| 8 | UC08 | Configure System Parameters | System Administrator | None | Set business rules, approval thresholds, and automated triggers. | High |
| 9 | UC09 | Perform Compliance Audit | Platform Auditor | Government Regulatory Portal | Inspect audit trails and ensure regulatory compliance. | Medium |
| 10 | UC10 | Manage System User Roles | System Administrator | None | Assign access permissions, roles, and security policies. | High |
| 11 | UC11 | Process Payment Transaction | Payment Gateway API | Platform User / Client | Execute billing charges, fee calculations, and electronic payouts. | High |
| 12 | UC12 | Dispatch Automated Alerts | Notification & SMS Gateway | Platform User / Client | Trigger automated SMS, push notifications, and email notices. | Medium |
| 13 | UC13 | Store & Verify Digital Documents | Document Storage & e-Sign API | Service Provider / Agent | Archive PDF contracts, proof documents, and compute hash verification. | High |
| 14 | UC14 | Verify Regulatory Compliance | Government Regulatory Portal | System Administrator | Query government databases for license checks and regulatory approvals. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Submit Home Renovation & Contractor Matching Request |
| **Actor(s)** | Primary: Platform User / Client / Secondary: Notification & SMS Gateway |
| **Description** | Allows Platform User / Client to submit a formal request or transaction within Home Renovation & Contractor Matching System with validated inputs. |
| **Precondition** | 1. User is authenticated and logged into the system.<br>2. System services are active and accessible. |
| **Main Flow** | 1. User accesses the request submission page.<br>2. System presents input form tailored for Home Renovation & Contractor Matching System.<br>3. User enters required details, uploads supporting files.<br>4. System validates inputs against business rules and data schemas.<br>5. User reviews summary and confirms submission.<br>6. System stores record with status 'PENDING_REVIEW'.<br>7. System triggers Notification & SMS Gateway to send confirmation notice to User. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Process Home Renovation & Contractor Matching Application |
| **Actor(s)** | Primary: Service Provider / Agent / Secondary: Document Storage & e-Sign API |
| **Description** | Allows Service Provider / Agent to review, inspect, and process user requests. |
...
| **Actor(s)** | Primary: System Administrator / Secondary: Government Regulatory Portal |
| **Description** | Enables System Administrator to issue final executive approval or authorization for high-priority items. |
...
| **Actor(s)** | Primary: Payment Gateway API / Secondary: Platform User / Client |
