# Use Case Diagram — Homeowner Association (HOA) Management System
...
flowchart LR
    ...
    subgraph SystemBoundary["Homeowner Association (HOA) Management System"]
        UC01(["Submit Homeowner Association (HOA) Management Request"])
        UC02(["View Homeowner Association (HOA) Management Dashboard"])
        UC03(["Manage Profile & Settings"])
        UC04(["Process Homeowner Association (HOA) Management Application"])
...
|---|-------|------------|------------------|-------------------|
| 1 | Resident / Occupant | Primary | Main user initiating homeowner association (hoa) management system operations. | UC01, UC02, UC03, UC11, UC12 |
| 2 | Facility Manager | Primary | Handles operational workflows in homeowner association (hoa) management system. | UC04, UC05, UC06, UC13 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Homeowner Association (HOA) Management Request | Resident / Occupant | Notification & SMS Gateway | Initiate a new request or operation within Homeowner Association (HOA) Management System. | High |
| 2 | UC02 | View Homeowner Association (HOA) Management Dashboard | Resident / Occupant | None | Display overview summary metrics, pending actions, and status updates. | High |
...
| 4 | UC04 | Process Homeowner Association (HOA) Management Application | Facility Manager | Document Storage & e-Sign API | Evaluate submitted data, verify documents, and perform operational tasks. | High |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Submit Homeowner Association (HOA) Management Request |
| **Actor(s)** | Primary: Resident / Occupant / Secondary: Notification & SMS Gateway |
| **Description** | Allows Resident / Occupant to submit a formal request or transaction within Homeowner Association (HOA) Management System with validated inputs. |
| **Precondition** | 1. User is authenticated and logged into the system.<br>2. System services are active and accessible. |
| **Main Flow** | 1. User accesses the request submission page.<br>2. System presents input form tailored for Homeowner Association (HOA) Management System.<br>3. User enters required details, uploads supporting files.<br>4. System validates inputs against business rules and data schemas.<br>5. User reviews summary and confirms submission.<br>6. System stores record with status 'PENDING_REVIEW'.<br>7. System triggers Notification & SMS Gateway to send confirmation notice to User. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Process Homeowner Association (HOA) Management Application |
