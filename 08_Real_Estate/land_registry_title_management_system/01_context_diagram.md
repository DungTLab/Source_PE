# Context Diagram — Land Registry & Title Management System

## Mermaid Code

```mermaid
flowchart LR
    Actor1["Applicant / Property Owner"]
    Actor2["Valation Specialist / Officer"]
    Actor3["Compliance Auditor"]
    ...
    System(("System Name: Land Registry & Title Management System"))
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Applicant / Property Owner | Primary | service request, input parameters, verification documents | status confirmation, processing results, system alerts | Primary end user interacting with core workflows. |
| 2 | Valation Specialist / Officer | Primary | operation logs, updated parameters, execution status | assigned tasks, client requests, performance reports | Operational role managing tasks and service requests. |
| 3 | Compliance Auditor | Primary | approval decision, configuration parameters, override commands | approval queue, system metrics, compliance reports | Administrative user overseeing compliance and settings. |
| 4 | Finance Manager | Primary | audit query, report parameters, policy update | audit logs, aggregate metrics, anomaly alerts | Supervisory role checking quality control. |
