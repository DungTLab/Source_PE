# Use Case Diagram — Land Registry & Title Management System

## Mermaid Code

```mermaid
flowchart LR
    actor_1["Applicant / Property Owner"]
    actor_2["Valation Specialist / Officer"]
    actor_3["Compliance Auditor"]
    actor_4["Finance Manager"]
    ...
    subgraph SystemBoundary["Land Registry & Title Management System"]
        UC01(["Submit Land Registry & Title Management Request"])
        UC02(["View Land Registry & Title Management Dashboard"])
        UC03(["Manage Profile & Settings"])
        UC04(["Process Land Registry & Title Management Application"])
...
|---|-------|------------|------------------|-------------------|
| 1 | Applicant / Property Owner | Primary | Main user initiating land registry & title management system operations. | UC01, UC02, UC03, UC11, UC12 |
| 2 | Valation Specialist / Officer | Primary | Handles operational workflows in land registry & title management system. | UC04, UC05, UC06, UC13 |
| 3 | Compliance Auditor | Primary | Manages system configurations and governance. | UC07, UC08, UC10, UC14 |
| 4 | Finance Manager | Primary | Monitors platform integrity and audit compliance. | UC09 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Land Registry & Title Management Request | Applicant / Property Owner | Notification & SMS Gateway | Initiate a new request or operation within Land Registry & Title Management System. | High |
| 2 | UC02 | View Land Registry & Title Management Dashboard | Applicant / Property Owner | None | Display overview summary metrics, pending actions, and status updates. | High |
| 3 | UC03 | Manage Profile & Settings | Applicant / Property Owner | None | Update account information, notification settings, and preferences. | Medium |
| 4 | UC04 | Process Land Registry & Title Management Application | Valation Specialist / Officer | Document Storage & e-Sign API | Evaluate submitted data, verify documents, and perform operational tasks. | High |
| 5 | UC05 | Update Status & Workflow State | Valation Specialist / Officer | Notification & SMS Gateway | Advance record through lifecycle states (Pending, Active, Closed). | High |
| 6 | UC06 | Generate Operational Reports | Valation Specialist / Officer | None | Extract detailed summary statistics, operational metrics, and logs. | Medium |
| 7 | UC07 | Approve Operational Submission | Compliance Auditor | Government Regulatory Portal | Review and approve/reject high-level operational or legal requests. | High |
| 8 | UC08 | Configure System Parameters | Compliance Auditor | None | Set business rules, approval thresholds, and automated triggers. | High |
| 9 | UC09 | Perform Compliance Audit | Finance Manager | Government Regulatory Portal | Inspect audit trails and ensure regulatory compliance. | Medium |
| 10 | UC10 | Manage System User Roles | Compliance Auditor | None | Assign access permissions, roles, and security policies. | High |
| 11 | UC11 | Process Payment Transaction | Payment Gateway API | Applicant / Property Owner | Execute billing charges, fee calculations, and electronic payouts. | High |
| 12 | UC12 | Dispatch Automated Alerts | Notification & SMS Gateway | Applicant / Property Owner | Trigger automated SMS, push notifications, and email notices. | Medium |
| 13 | UC13 | Store & Verify Digital Documents | Document Storage & e-Sign API | Valation Specialist / Officer | Archive PDF contracts, proof documents, and compute hash verification. | High |
| 14 | UC14 | Verify Regulatory Compliance | Government Regulatory Portal | Compliance Auditor | Query government databases for license checks and regulatory approvals. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Submit Land Registry & Title Management Request |
| **Actor(s)** | Primary: Applicant / Property Owner / Secondary: Notification & SMS Gateway |
| **Description** | Allows Applicant / Property Owner to submit a formal request or transaction within Land Registry & Title Management System with validated inputs. |
| **Precondition** | 1. User is authenticated and logged into the system.<br>2. System services are active and accessible. |
| **Main Flow** | 1. User accesses the request submission page.<br>2. System presents input form tailored for Land Registry & Title Management System.<br>3. User enters required details, uploads supporting files.<br>4. System validates inputs against business rules and data schemas.<br>5. User reviews summary and confirms submission.<br>6. System stores record with status 'PENDING_REVIEW'.<br>7. System triggers Notification & SMS Gateway to send confirmation notice to User. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Process Land Registry & Title Management Application |
| **Actor(s)** | Primary: Valation Specialist / Officer / Secondary: Document Storage & e-Sign API |
| **Description** | Allows Valation Specialist / Officer to review, inspect, and process user requests. |
...
| **Actor(s)** | Primary: Compliance Auditor / Secondary: Government Regulatory Portal |
| **Description** | Enables Compliance Auditor to issue final executive approval or authorization for high-priority items. |
...
| **Actor(s)** | Primary: Payment Gateway API / Secondary: Applicant / Property Owner |
