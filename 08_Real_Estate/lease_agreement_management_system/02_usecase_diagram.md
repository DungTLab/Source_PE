# Use Case Diagram — Lease Agreement Management System
...
flowchart LR
    ...
    subgraph SystemBoundary["Lease Agreement Management System"]
        UC01(["Submit Lease Agreement Management Request"])
        UC02(["View Lease Agreement Management Dashboard"])
        UC03(["Manage Profile & Settings"])
        UC04(["Process Lease Agreement Management Application"])
...
|---|-------|------------|------------------|-------------------|
| 1 | Project Manager | Primary | Main user initiating lease agreement management system operations. | UC01, UC02, UC03, UC11, UC12 |
| 2 | Asset Manager | Primary | Handles operational workflows in lease agreement management system. | UC04, UC05, UC06, UC13 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Lease Agreement Management Request | Project Manager | Notification & SMS Gateway | Initiate a new request or operation within Lease Agreement Management System. | High |
| 2 | UC02 | View Lease Agreement Management Dashboard | Project Manager | None | Display overview summary metrics, pending actions, and status updates. | High |
...
| 4 | UC04 | Process Lease Agreement Management Application | Asset Manager | Document Storage & e-Sign API | Evaluate submitted data, verify documents, and perform operational tasks. | High |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Submit Lease Agreement Management Request |
| **Actor(s)** | Primary: Project Manager / Secondary: Notification & SMS Gateway |
| **Description** | Allows Project Manager to submit a formal request or transaction within Lease Agreement Management System with validated inputs. |
| **Precondition** | 1. User is authenticated and logged into the system.<br>2. System services are active and accessible. |
| **Main Flow** | 1. User accesses the request submission page.<br>2. System presents input form tailored for Lease Agreement Management System.<br>3. User enters required details, uploads supporting files.<br>4. System validates inputs against business rules and data schemas.<br>5. User reviews summary and confirms submission.<br>6. System stores record with status 'PENDING_REVIEW'.<br>7. System triggers Notification & SMS Gateway to send confirmation notice to User. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Process Lease Agreement Management Application |
