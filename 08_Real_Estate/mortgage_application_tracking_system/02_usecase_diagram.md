# Use Case Diagram — Mortgage Application & Tracking System
...
flowchart LR
    ...
    subgraph SystemBoundary["Mortgage Application & Tracking System"]
        UC01(["Submit Mortgage Application & Tracking Request"])
        UC02(["View Mortgage Application & Tracking Dashboard"])
        UC03(["Manage Profile & Settings"])
        UC04(["Process Mortgage Application & Tracking Application"])
...
|---|-------|------------|------------------|-------------------|
| 1 | Applicant / Property Owner | Primary | Main user initiating mortgage application & tracking system operations. | UC01, UC02, UC03, UC11, UC12 |
| 2 | Valation Specialist / Officer | Primary | Handles operational workflows in mortgage application & tracking system. | UC04, UC05, UC06, UC13 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Mortgage Application & Tracking Request | Applicant / Property Owner | Notification & SMS Gateway | Initiate a new request or operation within Mortgage Application & Tracking System. | High |
| 2 | UC02 | View Mortgage Application & Tracking Dashboard | Applicant / Property Owner | None | Display overview summary metrics, pending actions, and status updates. | High |
...
| 4 | UC04 | Process Mortgage Application & Tracking Application | Valation Specialist / Officer | Document Storage & e-Sign API | Evaluate submitted data, verify documents, and perform operational tasks. | High |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Submit Mortgage Application & Tracking Request |
| **Actor(s)** | Primary: Applicant / Property Owner / Secondary: Notification & SMS Gateway |
| **Description** | Allows Applicant / Property Owner to submit a formal request or transaction within Mortgage Application & Tracking System with validated inputs. |
| **Precondition** | 1. User is authenticated and logged into the system.<br>2. System services are active and accessible. |
| **Main Flow** | 1. User accesses the request submission page.<br>2. System presents input form tailored for Mortgage Application & Tracking System.<br>3. User enters required details, uploads supporting files.<br>4. System validates inputs against business rules and data schemas.<br>5. User reviews summary and confirms submission.<br>6. System stores record with status 'PENDING_REVIEW'.<br>7. System triggers Notification & SMS Gateway to send confirmation notice to User. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Process Mortgage Application & Tracking Application |
