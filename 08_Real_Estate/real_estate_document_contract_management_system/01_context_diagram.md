# Context Diagram — Real Estate Document & Contract Management System
...
flowchart LR
    ...
    Actor2["Asset Manager"]
    Actor3["Operational Staff"]
    Actor4["Executive Administrator"]
    ...
    System(("System Name: Real Estate Document & Contract Management System"))
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Project Manager | Primary | service request, input parameters, verification documents | status confirmation, processing results, system alerts | Primary end user interacting with core workflows. |
| 2 | Asset Manager | Primary | operation logs, updated parameters, execution status | assigned tasks, client requests, performance reports | Operational role managing tasks and service requests. |
| 3 | Operational Staff | Primary | approval decision, configuration parameters, override commands | approval queue, system metrics, compliance reports | Administrative user overseeing compliance and settings. |
| 4 | Executive Administrator | Primary | audit query, report parameters, policy update | audit logs, aggregate metrics, anomaly alerts | Supervisory role checking quality control. |
