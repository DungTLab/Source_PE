# Use Case Diagram — Real Estate Escrow Management System
...
flowchart LR
    ...
    subgraph SystemBoundary["Real Estate Escrow Management System"]
        UC01(["Submit Real Estate Escrow Management Request"])
        UC02(["View Real Estate Escrow Management Dashboard"])
        UC03(["Manage Profile & Settings"])
        UC04(["Process Real Estate Escrow Management Application"])
...
|---|-------|------------|------------------|-------------------|
| 1 | Applicant / Property Owner | Primary | Main user initiating real estate escrow management system operations. | UC01, UC02, UC03, UC11, UC12 |
| 2 | Valation Specialist / Officer | Primary | Handles operational workflows in real estate escrow management system. | UC04, UC05, UC06, UC13 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Real Estate Escrow Management Request | Applicant / Property Owner | Notification & SMS Gateway | Initiate a new request or operation within Real Estate Escrow Management System. | High |
| 2 | UC02 | View Real Estate Escrow Management Dashboard | Applicant / Property Owner | None | Display overview summary metrics, pending actions, and status updates. | High |
...
| 4 | UC04 | Process Real Estate Escrow Management Application | Valation Specialist / Officer | Document Storage & e-Sign API | Evaluate submitted data, verify documents, and perform operational tasks. | High |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Submit Real Estate Escrow Management Request |
| **Actor(s)** | Primary: Applicant / Property Owner / Secondary: Notification & SMS Gateway |
| **Description** | Allows Applicant / Property Owner to submit a formal request or transaction within Real Estate Escrow Management System with validated inputs. |
| **Precondition** | 1. User is authenticated and logged into the system.<br>2. System services are active and accessible. |
| **Main Flow** | 1. User accesses the request submission page.<br>2. System presents input form tailored for Real Estate Escrow Management System.<br>3. User enters required details, uploads supporting files.<br>4. System validates inputs against business rules and data schemas.<br>5. User reviews summary and confirms submission.<br>6. System stores record with status 'PENDING_REVIEW'.<br>7. System triggers Notification & SMS Gateway to send confirmation notice to User. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Process Real Estate Escrow Management Application |
