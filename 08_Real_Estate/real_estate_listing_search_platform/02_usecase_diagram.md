# Use Case Diagram — Real Estate Listing & Search Platform
...
flowchart LR
    ...
    subgraph SystemBoundary["Real Estate Listing & Search Platform"]
        UC01(["Submit Real Estate Listing & Search Request"])
        UC02(["View Real Estate Listing & Search Dashboard"])
        UC03(["Manage Profile & Settings"])
        UC04(["Process Real Estate Listing & Search Application"])
...
|---|-------|------------|------------------|-------------------|
| 1 | Platform User / Client | Primary | Main user initiating real estate listing & search platform operations. | UC01, UC02, UC03, UC11, UC12 |
| 2 | Service Provider / Agent | Primary | Handles operational workflows in real estate listing & search platform. | UC04, UC05, UC06, UC13 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Real Estate Listing & Search Request | Platform User / Client | Notification & SMS Gateway | Initiate a new request or operation within Real Estate Listing & Search Platform. | High |
| 2 | UC02 | View Real Estate Listing & Search Dashboard | Platform User / Client | None | Display overview summary metrics, pending actions, and status updates. | High |
...
| 4 | UC04 | Process Real Estate Listing & Search Application | Service Provider / Agent | Document Storage & e-Sign API | Evaluate submitted data, verify documents, and perform operational tasks. | High |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Submit Real Estate Listing & Search Request |
| **Actor(s)** | Primary: Platform User / Client / Secondary: Notification & SMS Gateway |
| **Description** | Allows Platform User / Client to submit a formal request or transaction within Real Estate Listing & Search Platform with validated inputs. |
| **Precondition** | 1. User is authenticated and logged into the system.<br>2. System services are active and accessible. |
| **Main Flow** | 1. User accesses the request submission page.<br>2. System presents input form tailored for Real Estate Listing & Search Platform.<br>3. User enters required details, uploads supporting files.<br>4. System validates inputs against business rules and data schemas.<br>5. User reviews summary and confirms submission.<br>6. System stores record with status 'PENDING_REVIEW'.<br>7. System triggers Notification & SMS Gateway to send confirmation notice to User. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Process Real Estate Listing & Search Application |
