# Use Case Diagram — Student Housing Management System
...
flowchart LR
    ...
    subgraph SystemBoundary["Student Housing Management System"]
        UC01(["Submit Student Housing Management Request"])
        UC02(["View Student Housing Management Dashboard"])
        UC03(["Manage Profile & Settings"])
        UC04(["Process Student Housing Management Application"])
...
|---|-------|------------|------------------|-------------------|
| 1 | Tenant | Primary | Main user initiating student housing management system operations. | UC01, UC02, UC03, UC11, UC12 |
| 2 | Property Owner / Landlord | Primary | Handles operational workflows in student housing management system. | UC04, UC05, UC06, UC13 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Student Housing Management Request | Tenant | Notification & SMS Gateway | Initiate a new request or operation within Student Housing Management System. | High |
| 2 | UC02 | View Student Housing Management Dashboard | Tenant | None | Display overview summary metrics, pending actions, and status updates. | High |
| 3 | UC03 | Manage Profile & Settings | Tenant | None | Update account information, notification settings, and preferences. | Medium |
| 4 | UC04 | Process Student Housing Management Application | Property Owner / Landlord | Document Storage & e-Sign API | Evaluate submitted data, verify documents, and perform operational tasks. | High |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Submit Student Housing Management Request |
| **Actor(s)** | Primary: Tenant / Secondary: Notification & SMS Gateway |
| **Description** | Allows Tenant to submit a formal request or transaction within Student Housing Management System with validated inputs. |
| **Precondition** | 1. User is authenticated and logged into the system.<br>2. System services are active and accessible. |
| **Main Flow** | 1. User accesses the request submission page.<br>2. System presents input form tailored for Student Housing Management System.<br>3. User enters required details, uploads supporting files.<br>4. System validates inputs against business rules and data schemas.<br>5. User reviews summary and confirms submission.<br>6. System stores record with status 'PENDING_REVIEW'.<br>7. System triggers Notification & SMS Gateway to send confirmation notice to User. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Process Student Housing Management Application |
