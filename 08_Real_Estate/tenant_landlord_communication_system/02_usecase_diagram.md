# Use Case Diagram — Tenant & Landlord Communication System
...
flowchart LR
    actor_1["Tenant"]
    actor_2["Property Owner / Landlord"]
    actor_3["Property Manager"]
    actor_4["Maintenance Staff"]
    ...
    subgraph SystemBoundary["Tenant & Landlord Communication System"]
        UC01(["Submit Tenant & Landlord Communication Request"])
        UC02(["View Tenant & Landlord Communication Dashboard"])
        UC03(["Manage Profile & Settings"])
        UC04(["Process Tenant & Landlord Communication Application"])
...
|---|-------|------------|------------------|-------------------|
| 1 | Tenant | Primary | Main user initiating tenant & landlord communication system operations. | UC01, UC02, UC03, UC11, UC12 |
| 2 | Property Owner / Landlord | Primary | Handles operational workflows in tenant & landlord communication system. | UC04, UC05, UC06, UC13 |
| 3 | Property Manager | Primary | Manages system configurations and governance. | UC07, UC08, UC10, UC14 |
| 4 | Maintenance Staff | Primary | Monitors platform integrity and audit compliance. | UC09 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Submit Tenant & Landlord Communication Request | Tenant | Notification & SMS Gateway | Initiate a new request or operation within Tenant & Landlord Communication System. | High |
| 2 | UC02 | View Tenant & Landlord Communication Dashboard | Tenant | None | Display overview summary metrics, pending actions, and status updates. | High |
| 3 | UC03 | Manage Profile & Settings | Tenant | None | Update account information, notification settings, and preferences. | Medium |
| 4 | UC04 | Process Tenant & Landlord Communication Application | Property Owner / Landlord | Document Storage & e-Sign API | Evaluate submitted data, verify documents, and perform operational tasks. | High |
| 5 | UC05 | Update Status & Workflow State | Property Owner / Landlord | Notification & SMS Gateway | Advance record through lifecycle states (Pending, Active, Closed). | High |
| 6 | UC06 | Generate Operational Reports | Property Owner / Landlord | None | Extract detailed summary statistics, operational metrics, and logs. | Medium |
| 7 | UC07 | Approve Operational Submission | Property Manager | Government Regulatory Portal | Review and approve/reject high-level operational or legal requests. | High |
| 8 | UC08 | Configure System Parameters | Property Manager | None | Set business rules, approval thresholds, and automated triggers. | High |
| 9 | UC09 | Perform Compliance Audit | Maintenance Staff | Government Regulatory Portal | Inspect audit trails and ensure regulatory compliance. | Medium |
| 10 | UC10 | Manage System User Roles | Property Manager | None | Assign access permissions, roles, and security policies. | High |
| 11 | UC11 | Process Payment Transaction | Payment Gateway API | Tenant | Execute billing charges, fee calculations, and electronic payouts. | High |
| 12 | UC12 | Dispatch Automated Alerts | Notification & SMS Gateway | Tenant | Trigger automated SMS, push notifications, and email notices. | Medium |
| 13 | UC13 | Store & Verify Digital Documents | Document Storage & e-Sign API | Property Owner / Landlord | Archive PDF contracts, proof documents, and compute hash verification. | High |
| 14 | UC14 | Verify Regulatory Compliance | Government Regulatory Portal | Property Manager | Query government databases for license checks and regulatory approvals. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Submit Tenant & Landlord Communication Request |
| **Actor(s)** | Primary: Tenant / Secondary: Notification & SMS Gateway |
| **Description** | Allows Tenant to submit a formal request or transaction within Tenant & Landlord Communication System with validated inputs. |
| **Precondition** | 1. User is authenticated and logged into the system.<br>2. System services are active and accessible. |
| **Main Flow** | 1. User accesses the request submission page.<br>2. System presents input form tailored for Tenant & Landlord Communication System.<br>3. User enters required details, uploads supporting files.<br>4. System validates inputs against business rules and data schemas.<br>5. User reviews summary and confirms submission.<br>6. System stores record with status 'PENDING_REVIEW'.<br>7. System triggers Notification & SMS Gateway to send confirmation notice to User. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Process Tenant & Landlord Communication Application |
| **Actor(s)** | Primary: Property Owner / Landlord / Secondary: Document Storage & e-Sign API |
| **Description** | Allows Property Owner / Landlord to review, inspect, and process user requests. |
...
| **Actor(s)** | Primary: Property Manager / Secondary: Government Regulatory Portal |
| **Description** | Enables Property Manager to issue final executive approval or authorization for high-priority items. |
...
| **Actor(s)** | Primary: Payment Gateway API / Secondary: Tenant |
