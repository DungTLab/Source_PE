# Context Diagram — Automated Guided Vehicle (AGV) Management System

## Mermaid Code

```mermaid
flowchart LR
    Actor1["Automation Engineer"]
    Actor2["IoT Systems Operator"]
    Actor3["Plant Operations Director"]
    Actor4["Safety Manager"]
    ...
    System(("System Name: Automated Guided Vehicle (AGV) Management System"))
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Automation Engineer | Primary | production parameter, work order execution payload, sensor reading | work instruction, schedule alert, operational metrics | Primary user executing plant line workflows. |
| 2 | IoT Systems Operator | Primary | inspection result, fault log, downtime report | task dispatch, quality alert, calibration specs | Operational supervisor managing line execution. |
| 3 | Plant Operations Director | Primary | production plan sign-off, threshold adjustment, override code | plant KPI dashboard, yield report, bottleneck warning | Plant manager responsible for throughput and compliance. |
| 4 | Safety Manager | Primary | audit query, compliance checklist, regulatory report | audit trail, traceability logs, non-conformance records | Auditing role checking ISO/FDA/OSHA compliance. |
