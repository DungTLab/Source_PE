# Context Diagram — Equipment Maintenance & Reliability System

## Mermaid Code

```mermaid
flowchart LR
    Actor1["Maintenance Technician"]
    Actor2["Reliability Engineer"]
    Actor3["Plant Maintenance Manager"]
    Actor4["Equipment Operator"]
    ...
    System(("System Name: Equipment Maintenance & Reliability System"))
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Maintenance Technician | Primary | production parameter, work order execution payload, sensor reading | work instruction, schedule alert, operational metrics | Primary user executing plant line workflows. |
| 2 | Reliability Engineer | Primary | inspection result, fault log, downtime report | task dispatch, quality alert, calibration specs | Operational supervisor managing line execution. |
| 3 | Plant Maintenance Manager | Primary | production plan sign-off, threshold adjustment, override code | plant KPI dashboard, yield report, bottleneck warning | Plant manager responsible for throughput and compliance. |
| 4 | Equipment Operator | Primary | audit query, compliance checklist, regulatory report | audit trail, traceability logs, non-conformance records | Auditing role checking ISO/FDA/OSHA compliance. |
