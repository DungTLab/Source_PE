# Use Case Diagram — Equipment Maintenance & Reliability System

## Mermaid Code

```mermaid
flowchart LR
    actor_1["Maintenance Technician"]
    actor_2["Reliability Engineer"]
    actor_3["Plant Maintenance Manager"]
    actor_4["Equipment Operator"]
    ...
    subgraph SystemBoundary["Equipment Maintenance & Reliability System"]
        UC01(["Execute Equipment Maintenance & Reliability Line Operation"])
        UC02(["View Equipment Maintenance & Reliability Shop Floor Dashboard"])
        UC03(["Configure Line Parameters & Recipe"])
        UC04(["Process Equipment Maintenance & Reliability Quality Inspection"])
...
|---|-------|------------|------------------|-------------------|
| 1 | Maintenance Technician | Primary | Main user initiating equipment maintenance & reliability system operations. | UC01, UC02, UC03, UC11 |
| 2 | Reliability Engineer | Primary | Manages line execution in equipment maintenance & reliability system. | UC04, UC05, UC06, UC13 |
| 3 | Plant Maintenance Manager | Primary | Manages factory line parameters and executive reporting. | UC07, UC08, UC10, UC12, UC14 |
| 4 | Equipment Operator | Primary | Monitors regulatory adherence and quality standards. | UC09 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Execute Equipment Maintenance & Reliability Line Operation | Maintenance Technician | PLC & SCADA Controller System | Initiate line operations and data capture in Equipment Maintenance & Reliability System. | High |
| 2 | UC02 | View Equipment Maintenance & Reliability Shop Floor Dashboard | Maintenance Technician | None | Display real-time line metrics, machine statuses, and production output. | High |
| 3 | UC03 | Configure Line Parameters & Recipe | Maintenance Technician | PLC & SCADA Controller System | Set machine speed, temperature, pressure setpoints, and job parameters. | Medium |
| 4 | UC04 | Process Equipment Maintenance & Reliability Quality Inspection | Reliability Engineer | Industrial Barcode & RFID Gateway | Perform dimensional, visual, or laboratory quality inspection tasks. | High |
| 5 | UC05 | Log Line Downtime & Fault Incident | Reliability Engineer | PLC & SCADA Controller System | Record machine breakdown reasons, duration, and corrective actions. | High |
| 6 | UC06 | Generate Line Yield & OEE Report | Reliability Engineer | Enterprise ERP System API | Compile Overall Equipment Effectiveness (OEE), scrap rates, and output logs. | Medium |
| 7 | UC07 | Approve Production Batch & Order | Plant Maintenance Manager | Industrial Safety & Standards Regulatory Portal | Review batch records and authorize final release to warehouse/shipping. | High |
| 8 | UC08 | Set Plant KPI & Safety Thresholds | Plant Maintenance Manager | None | Define tolerance limits, SPC control boundaries, and alert escalation rules. | High |
| 9 | UC09 | Audit Line Traceability & Compliance | Equipment Operator | Industrial Safety & Standards Regulatory Portal | Perform forward/backward batch traceability and compliance checks. | Medium |
| 10 | UC10 | Manage Shift Rosters & Workstation Roles | Plant Maintenance Manager | None | Assign operators to workstation lines and grant machine clearance. | High |
| 11 | UC11 | Stream Real-time PLC Telemetry Data | PLC & SCADA Controller System | Maintenance Technician | Continuous ingestion of temperature, vibration, and cycle data from equipment. | High |
| 12 | UC12 | Sync Orders with Enterprise ERP | Enterprise ERP System API | Plant Maintenance Manager | Synchronize raw material issues, finished goods counts, and BOM changes. | Medium |
| 13 | UC13 | Capture Barcode & RFID Lot Tag | Industrial Barcode & RFID Gateway | Reliability Engineer | Scan component serial numbers and associate them with production lot. | High |
| 14 | UC14 | Validate Regulatory Safety Standards | Industrial Safety & Standards Regulatory Portal | Plant Maintenance Manager | Submit environmental, worker safety, and ISO compliance metrics. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Execute Equipment Maintenance & Reliability Line Operation |
| **Actor(s)** | Primary: Maintenance Technician / Secondary: PLC & SCADA Controller System |
| **Description** | Allows Maintenance Technician to start, monitor, and execute line operations for a scheduled work order. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Process Equipment Maintenance & Reliability Quality Inspection |
| **Actor(s)** | Primary: Reliability Engineer / Secondary: Industrial Barcode & RFID Gateway |
| **Description** | Allows Reliability Engineer to record quality inspection results, measurements, and defect classifications for produced units. |
...
| **Actor(s)** | Primary: Plant Maintenance Manager / Secondary: Industrial Safety & Standards Regulatory Portal |
| **Description** | Enables Plant Maintenance Manager to review full batch genealogy, quality records, and authorize final product release. |
...
| **Actor(s)** | Primary: PLC & SCADA Controller System / Secondary: Maintenance Technician |
