# Conceptual ERD — Textile Manufacturing Management System
...
erDiagram
    ...
    PRODUCT_CATALOG ||--o{ TEXTILE_WORK_ORDER : "defines"
    USER_OPERATOR ||--o{ TEXTILE_WORK_ORDER : "executes"
    TEXTILE_WORK_ORDER ||--o{ BATCH_RUN : "divides_into"
    ...
    TEXTILE_WORK_ORDER }o--o{ EQUIPMENT_ASSET : "assigned_to"
```
...
| 2 | TEXTILE_WORK_ORDER | Đơn hàng Sản xuất | Master production work order tracking product, target quantity, line, and status in Textile Manufacturing Management System. | order_id PK, order_number, product_id FK, planned_qty, completed_qty, scrap_qty, line_id FK, status, start_time, end_time | belongs to PRODUCT_CATALOG, has many BATCH_RUN, has many QUALITY_INSPECTION |
| 3 | PRODUCT_CATALOG | Danh mục Sản phẩm / SKU | Finished goods, assemblies, and SKU specifications. | product_id PK, sku_code, product_name, revision_id, target_cycle_time_sec, standard_bom_id FK | has many TEXTILE_WORK_ORDER |
...
| 5 | BATCH_RUN | Lô Sản xuất | Granular production batch run details, lot numbers, and execution logs. | batch_id PK, order_id FK, batch_number, start_time, end_time, total_yield, status | belongs to TEXTILE_WORK_ORDER, has many QUALITY_INSPECTION |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | PRODUCT_CATALOG | ||--o{ | TEXTILE_WORK_ORDER | defines | Một mã sản phẩm SKU có thể được sản xuất qua nhiều đơn hàng sản xuất. |
| 2 | USER_OPERATOR | ||--o{ | TEXTILE_WORK_ORDER | executes | Người vận hành thực thi công việc trên các đơn hàng sản xuất. |
| 3 | TEXTILE_WORK_ORDER | ||--o{ | BATCH_RUN | divides_into | Một đơn hàng sản xuất được chia thành nhiều lô sản xuất. |
...
| 8 | TEXTILE_WORK_ORDER | }o--o{ | EQUIPMENT_ASSET | assigned_to | Đơn hàng sản xuất được gán vào các dây chuyền thiết bị. |
