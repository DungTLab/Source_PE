# Use Case Diagram — Boutique Hotel Management System
...
flowchart LR
    ...
    subgraph SystemBoundary[" Boutique Hotel Management System "]
        UC01(["Register Boutique Request"])
        UC02(["Process Boutique Transaction"])
        UC03(["Update Boutique Status"])
        UC04(["Approve Boutique Request"])
        UC05(["Generate Boutique Report"])
        UC06(["Configure Boutique Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Boutique History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Boutique Task"])
        UC11(["Audit Boutique Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Boutique Request | Operations Staff | Hotel Guest | Create new operational request in Boutique Hotel Management System. | High |
| 2 | UC02 | Process Boutique Transaction | Operations Staff | Payment Gateway | Execute payment or billing charge for Boutique Hotel Management System. | High |
| 3 | UC03 | Update Boutique Status | Operations Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Boutique Request | Department Manager | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Boutique Report | Department Manager | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Boutique Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search Boutique History | Operations Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Boutique Task | Operations Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Boutique Logs | Department Manager | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Boutique Request |
| **Actor(s)** | Primary: Operations Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Boutique Hotel Management System. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Boutique Transaction |
| **Actor(s)** | Primary: Operations Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Boutique Hotel Management System. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Boutique Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Boutique Request |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Boutique Report |
