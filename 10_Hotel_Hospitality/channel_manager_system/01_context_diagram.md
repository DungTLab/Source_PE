# Context Diagram — Channel Manager System (Hotel)
...
flowchart LR
    ...
    System((" Channel Manager System (Hotel) "))
...
|---|-------|------------|---------------------|---------------------------|-----------------|
| 1 | Operations Staff | Primary | Operation requests, status updates, transaction entries | Task assignments, system validations, operational logs | Nhân viên vận hành chính của Channel Manager System (Hotel) |
| 2 | Department Manager | Primary | Approval requests, strategy configs, policy updates | Performance reports, exception alerts, operational dashboards | Quản lý cấp cao giám sát hoạt động của Channel Manager System (Hotel) |
...
Hệ thống Channel Manager System (Hotel) chịu trách nhiệm xử lý toàn bộ quy trình nghiệp vụ chuyên sâu thuộc lĩnh vực Distribution. Hệ thống quản lý dữ liệu đầu vào từ Operations Staff và Hotel Guest, đồng bộ hóa tài khoản và giao dịch với PMS Central Database, đồng thời ủy quyền xử lý thanh toán tài chính cho Payment Gateway và gửi thông báo qua Notification Service.
