# Context Diagram — Concierge & Guest Service System

## Mermaid Code

```mermaid
flowchart LR
    Actor1["Concierge Staff"]
    Actor2["Head Concierge"]
    Actor3["Hotel Guest"]
    ...
    System((" Concierge & Guest Service System "))
...
|---|-------|------------|---------------------|---------------------------|-----------------|
| 1 | Concierge Staff | Primary | Operation requests, status updates, transaction entries | Task assignments, system validations, operational logs | Nhân viên vận hành chính của Concierge & Guest Service System |
| 2 | Head Concierge | Primary | Approval requests, strategy configs, policy updates | Performance reports, exception alerts, operational dashboards | Quản lý cấp cao giám sát hoạt động của Concierge & Guest Service System |
...
Hệ thống Concierge & Guest Service System chịu trách nhiệm xử lý toàn bộ quy trình nghiệp vụ chuyên sâu thuộc lĩnh vực Concierge. Hệ thống quản lý dữ liệu đầu vào từ Concierge Staff và Hotel Guest, đồng bộ hóa tài khoản và giao dịch với PMS Central Database, đồng thời ủy quyền xử lý thanh toán tài chính cho Payment Gateway và gửi thông báo qua Notification Service.
