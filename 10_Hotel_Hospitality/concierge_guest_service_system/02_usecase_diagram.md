# Use Case Diagram — Concierge & Guest Service System

## Mermaid Code

```mermaid
flowchart LR
    actor_Concierge_Staff["Concierge Staff"]
    actor_Hotel_Guest["Hotel Guest"]
    actor_Payment_Gateway["Payment Gateway"]
    actor_PMS_Central_Database["PMS Central Database"]
    actor_Head_Concierge["Head Concierge"]
    ...
    subgraph SystemBoundary[" Concierge & Guest Service System "]
        UC01(["Register Concierge Request"])
        UC02(["Process Concierge Transaction"])
        UC03(["Update Concierge Status"])
        UC04(["Approve Concierge Request"])
        UC05(["Generate Concierge Report"])
        UC06(["Configure Concierge Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Concierge History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Concierge Task"])
        UC11(["Audit Concierge Logs"])
        UC12(["Export Performance Analytics"])
    end

    actor_Concierge_Staff --> UC01
    UC01 --> actor_Hotel_Guest
    actor_Concierge_Staff --> UC02
    UC02 --> actor_Payment_Gateway
    actor_Concierge_Staff --> UC03
    UC03 --> actor_PMS_Central_Database
    actor_Head_Concierge --> UC04
    actor_Head_Concierge --> UC05
    UC05 --> actor_Audit___Compliance_System
    actor_System_Administrator --> UC06
    actor_Concierge_Staff --> UC07
    UC07 --> actor_Notification_Service
    actor_Concierge_Staff --> UC08
    actor_PMS_Central_Database --> UC09
    actor_Concierge_Staff --> UC10
    UC10 --> actor_Hotel_Guest
    actor_Head_Concierge --> UC11
    UC11 --> actor_Audit___Compliance_System
    actor_Head_Concierge --> UC12
    UC01 -.->|"<<include>>"| UC02
    UC03 -.->|"<<extend>>"| UC04
```
...
|---|-------|------------|------------------|-------------------|
| 1 | Concierge Staff | Primary | Phụ trách các tác vụ liên quan đến Concierge Staff trong hệ thống | UC01, UC02, UC03, UC07, UC08, UC10 |
...
| 5 | Head Concierge | Primary | Phụ trách các tác vụ liên quan đến Head Concierge trong hệ thống | UC04, UC05, UC11, UC12 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Concierge Request | Concierge Staff | Hotel Guest | Create new operational request in Concierge & Guest Service System. | High |
| 2 | UC02 | Process Concierge Transaction | Concierge Staff | Payment Gateway | Execute payment or billing charge for Concierge & Guest Service System. | High |
| 3 | UC03 | Update Concierge Status | Concierge Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Concierge Request | Head Concierge | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Concierge Report | Head Concierge | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Concierge Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
| 7 | UC07 | Send Service Notification | Concierge Staff | Notification Service | Dispatch automated alerts to guests or staff. | Medium |
| 8 | UC08 | Search Concierge History | Concierge Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Concierge Task | Concierge Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Concierge Logs | Head Concierge | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
| 12 | UC12 | Export Performance Analytics | Head Concierge | None | Export operational data to Excel or PDF format. | Low |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Concierge Request |
| **Actor(s)** | Primary: Concierge Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Concierge & Guest Service System. |
| **Precondition** | 1. Người dùng đã đăng nhập hệ thống với quyền hạn hợp lệ.<br>2. Danh mục dịch vụ và khách hàng đã sẵn sàng. |
| **Main Flow** | 1. Concierge Staff truy cập màn hình tạo yêu cầu dịch vụ mới.<br>2. System hiển thị biểu mẫu nhập liệu chuẩn hóa.<br>3. Concierge Staff chọn thông tin Hotel Guest và chi tiết dịch vụ yêu cầu.<br>4. System kiểm tra tính hợp lệ của thông tin và tình trạng khả dụng.<br>5. Concierge Staff nhấn Xác nhận khởi tạo yêu cầu.<br>6. System tạo mới hồ sơ, lưu vào CSDL và gửi thông báo xác nhận. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Concierge Transaction |
| **Actor(s)** | Primary: Concierge Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Concierge & Guest Service System. |
...
| **Main Flow** | 1. Concierge Staff mở yêu cầu cần thanh toán và kiểm tra bảng chi tiết chi phí.<br>2. System tổng hợp giá dịch vụ, thuế VAT và phí phục vụ.<br>3. Concierge Staff chọn phương thức thanh toán (Ghi nợ phòng Folio / Tiền mặt / Thẻ ngân hàng).<br>4. System gửi lệnh tính phí sang Folio PMS hoặc Payment Gateway.<br>5. Payment Gateway trả về kết quả giao dịch thành công.<br>6. System cập nhật trạng thái đơn dịch vụ thành Paid và in hóa đơn thanh toán. |
| **Alternative Flow** | AF1 — Ghi nợ về tài khoản công ty: Concierge Staff chọn nợ công ty, kiểm tra hạn mức nợ hợp lệ.<br>AF2 — Thanh toán bằng voucher: System trừ giá trị voucher trước khi tính số tiền còn lại. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Concierge Status |
| **Actor(s)** | Primary: Concierge Staff / Secondary: PMS Central Database |
...
| **Main Flow** | 1. Concierge Staff mở danh sách công việc được phân công.<br>2. System hiển thị các yêu cầu đang thực hiện.<br>3. Concierge Staff chọn yêu cầu và thay đổi trạng thái sang In-Progress hoặc Completed.<br>4. System ghi nhận thời điểm cập nhật và nhân viên thực hiện.<br>5. System đồng bộ trạng thái mới sang PMS Central Database.<br>6. System gửi thông báo trạng thái mới tới khách hàng. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Concierge Request |
| **Actor(s)** | Primary: Head Concierge / Secondary: None |
...
| **Main Flow** | 1. Head Concierge truy cập danh sách yêu cầu chờ duyệt.<br>2. System hiển thị chi tiết nội dung, chi phí và lý do yêu cầu.<br>3. Head Concierge kiểm tra tính hợp lệ và hạn mức cho phép.<br>4. Head Concierge nhấn Phê duyệt (Approve).<br>5. System chuyển trạng thái yêu cầu sang Approved và chuyển tiếp cho nhân viên xử lý.<br>6. System lưu lại lịch sử phê duyệt của quản lý. |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Concierge Report |
| **Actor(s)** | Primary: Head Concierge / Secondary: Audit & Compliance System |
...
| **Main Flow** | 1. Head Concierge chọn phân hệ Báo cáo và chọn khoảng thời gian (Ngày/Tuần/Tháng).<br>2. System truy vấn dữ liệu vận hành từ CSDL trung tâm.<br>3. System tính toán các chỉ số KPI, tổng doanh thu và biểu đồ xu hướng.<br>4. System xuất màn hình báo cáo tổng quan trực quan.<br>5. Head Concierge chọn Tải báo cáo dạng PDF/Excel.<br>6. System gửi bản sao báo cáo tới Audit & Compliance System. |
