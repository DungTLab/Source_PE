# Use Case Diagram — Event & Conference Room Booking System
...
flowchart LR
    ...
    subgraph SystemBoundary[" Event & Conference Room Booking System "]
        UC01(["Register Events Request"])
        UC02(["Process Events Transaction"])
        UC03(["Update Events Status"])
        UC04(["Approve Events Request"])
        UC05(["Generate Events Report"])
        UC06(["Configure Events Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Events History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Events Task"])
        UC11(["Audit Events Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Events Request | Operations Staff | Hotel Guest | Create new operational request in Event & Conference Room Booking System. | High |
| 2 | UC02 | Process Events Transaction | Operations Staff | Payment Gateway | Execute payment or billing charge for Event & Conference Room Booking System. | High |
| 3 | UC03 | Update Events Status | Operations Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Events Request | Department Manager | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Events Report | Department Manager | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Events Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search Events History | Operations Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Events Task | Operations Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Events Logs | Department Manager | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Events Request |
| **Actor(s)** | Primary: Operations Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Event & Conference Room Booking System. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Events Transaction |
| **Actor(s)** | Primary: Operations Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Event & Conference Room Booking System. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Events Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Events Request |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Events Report |
