# Use Case Diagram — Guest Check-in Kiosk System
...
flowchart LR
    ...
    subgraph SystemBoundary[" Guest Check-in Kiosk System "]
        UC01(["Register Kiosk Request"])
        UC02(["Process Kiosk Transaction"])
        UC03(["Update Kiosk Status"])
        UC04(["Approve Kiosk Request"])
        UC05(["Generate Kiosk Report"])
        UC06(["Configure Kiosk Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Kiosk History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Kiosk Task"])
        UC11(["Audit Kiosk Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Kiosk Request | Operations Staff | Hotel Guest | Create new operational request in Guest Check-in Kiosk System. | High |
| 2 | UC02 | Process Kiosk Transaction | Operations Staff | Payment Gateway | Execute payment or billing charge for Guest Check-in Kiosk System. | High |
| 3 | UC03 | Update Kiosk Status | Operations Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Kiosk Request | Department Manager | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Kiosk Report | Department Manager | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Kiosk Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search Kiosk History | Operations Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Kiosk Task | Operations Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Kiosk Logs | Department Manager | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Kiosk Request |
| **Actor(s)** | Primary: Operations Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Guest Check-in Kiosk System. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Kiosk Transaction |
| **Actor(s)** | Primary: Operations Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Guest Check-in Kiosk System. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Kiosk Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Kiosk Request |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Kiosk Report |
