# Use Case Diagram — Hotel Analytics & Performance Dashboard
...
flowchart LR
    ...
    subgraph SystemBoundary[" Hotel Analytics & Performance Dashboard "]
        UC01(["Register Analytics Request"])
        UC02(["Process Analytics Transaction"])
        UC03(["Update Analytics Status"])
        UC04(["Approve Analytics Request"])
        UC05(["Generate Analytics Report"])
        UC06(["Configure Analytics Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Analytics History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Analytics Task"])
        UC11(["Audit Analytics Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Analytics Request | Operations Staff | Hotel Guest | Create new operational request in Hotel Analytics & Performance Dashboard. | High |
| 2 | UC02 | Process Analytics Transaction | Operations Staff | Payment Gateway | Execute payment or billing charge for Hotel Analytics & Performance Dashboard. | High |
| 3 | UC03 | Update Analytics Status | Operations Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Analytics Request | Department Manager | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Analytics Report | Department Manager | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Analytics Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search Analytics History | Operations Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Analytics Task | Operations Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Analytics Logs | Department Manager | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Analytics Request |
| **Actor(s)** | Primary: Operations Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Hotel Analytics & Performance Dashboard. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Analytics Transaction |
| **Actor(s)** | Primary: Operations Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Hotel Analytics & Performance Dashboard. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Analytics Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Analytics Request |
