# Use Case Diagram — Hotel CRM & Guest Loyalty System
...
flowchart LR
    ...
    subgraph SystemBoundary[" Hotel CRM & Guest Loyalty System "]
        UC01(["Register CRM Request"])
        UC02(["Process CRM Transaction"])
        UC03(["Update CRM Status"])
        UC04(["Approve CRM Request"])
        UC05(["Generate CRM Report"])
        UC06(["Configure CRM Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search CRM History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel CRM Task"])
        UC11(["Audit CRM Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register CRM Request | Operations Staff | Hotel Guest | Create new operational request in Hotel CRM & Guest Loyalty System. | High |
| 2 | UC02 | Process CRM Transaction | Operations Staff | Payment Gateway | Execute payment or billing charge for Hotel CRM & Guest Loyalty System. | High |
| 3 | UC03 | Update CRM Status | Operations Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve CRM Request | Department Manager | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate CRM Report | Department Manager | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure CRM Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search CRM History | Operations Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel CRM Task | Operations Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit CRM Logs | Department Manager | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register CRM Request |
| **Actor(s)** | Primary: Operations Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Hotel CRM & Guest Loyalty System. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process CRM Transaction |
| **Actor(s)** | Primary: Operations Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Hotel CRM & Guest Loyalty System. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update CRM Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve CRM Request |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate CRM Report |
