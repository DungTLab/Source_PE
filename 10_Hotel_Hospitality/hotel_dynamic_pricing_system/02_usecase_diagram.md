# Use Case Diagram — Hotel Dynamic Pricing System

## Mermaid Code

```mermaid
flowchart LR
    actor_Pricing_Analyst["Pricing Analyst"]
    actor_Hotel_Guest["Hotel Guest"]
    actor_Payment_Gateway["Payment Gateway"]
    actor_PMS_Central_Database["PMS Central Database"]
    actor_Revenue_Director["Revenue Director"]
    ...
    subgraph SystemBoundary[" Hotel Dynamic Pricing System "]
        UC01(["Register Dynamic Pricing Request"])
        UC02(["Process Dynamic Pricing Transaction"])
        UC03(["Update Dynamic Pricing Status"])
        UC04(["Approve Dynamic Pricing Request"])
        UC05(["Generate Dynamic Pricing Report"])
        UC06(["Configure Dynamic Pricing Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Dynamic Pricing History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Dynamic Pricing Task"])
        UC11(["Audit Dynamic Pricing Logs"])
        UC12(["Export Performance Analytics"])
    end

    actor_Pricing_Analyst --> UC01
    UC01 --> actor_Hotel_Guest
    actor_Pricing_Analyst --> UC02
    UC02 --> actor_Payment_Gateway
    actor_Pricing_Analyst --> UC03
    UC03 --> actor_PMS_Central_Database
    actor_Revenue_Director --> UC04
    actor_Revenue_Director --> UC05
    UC05 --> actor_Audit___Compliance_System
    actor_System_Administrator --> UC06
    actor_Pricing_Analyst --> UC07
    UC07 --> actor_Notification_Service
    actor_Pricing_Analyst --> UC08
    actor_PMS_Central_Database --> UC09
    actor_Pricing_Analyst --> UC10
    UC10 --> actor_Hotel_Guest
    actor_Revenue_Director --> UC11
    UC11 --> actor_Audit___Compliance_System
    actor_Revenue_Director --> UC12
    UC01 -.->|"<<include>>"| UC02
    UC03 -.->|"<<extend>>"| UC04
```
...
|---|-------|------------|------------------|-------------------|
| 1 | Pricing Analyst | Primary | Phụ trách các tác vụ liên quan đến Pricing Analyst trong hệ thống | UC01, UC02, UC03, UC07, UC08, UC10 |
...
| 5 | Revenue Director | Primary | Phụ trách các tác vụ liên quan đến Revenue Director trong hệ thống | UC04, UC05, UC11, UC12 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Dynamic Pricing Request | Pricing Analyst | Hotel Guest | Create new operational request in Hotel Dynamic Pricing System. | High |
| 2 | UC02 | Process Dynamic Pricing Transaction | Pricing Analyst | Payment Gateway | Execute payment or billing charge for Hotel Dynamic Pricing System. | High |
| 3 | UC03 | Update Dynamic Pricing Status | Pricing Analyst | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Dynamic Pricing Request | Revenue Director | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Dynamic Pricing Report | Revenue Director | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Dynamic Pricing Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
| 7 | UC07 | Send Service Notification | Pricing Analyst | Notification Service | Dispatch automated alerts to guests or staff. | Medium |
| 8 | UC08 | Search Dynamic Pricing History | Pricing Analyst | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Dynamic Pricing Task | Pricing Analyst | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Dynamic Pricing Logs | Revenue Director | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
| 12 | UC12 | Export Performance Analytics | Revenue Director | None | Export operational data to Excel or PDF format. | Low |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Dynamic Pricing Request |
| **Actor(s)** | Primary: Pricing Analyst / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Hotel Dynamic Pricing System. |
| **Precondition** | 1. Người dùng đã đăng nhập hệ thống với quyền hạn hợp lệ.<br>2. Danh mục dịch vụ và khách hàng đã sẵn sàng. |
| **Main Flow** | 1. Pricing Analyst truy cập màn hình tạo yêu cầu dịch vụ mới.<br>2. System hiển thị biểu mẫu nhập liệu chuẩn hóa.<br>3. Pricing Analyst chọn thông tin Hotel Guest và chi tiết dịch vụ yêu cầu.<br>4. System kiểm tra tính hợp lệ của thông tin và tình trạng khả dụng.<br>5. Pricing Analyst nhấn Xác nhận khởi tạo yêu cầu.<br>6. System tạo mới hồ sơ, lưu vào CSDL và gửi thông báo xác nhận. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Dynamic Pricing Transaction |
| **Actor(s)** | Primary: Pricing Analyst / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Hotel Dynamic Pricing System. |
...
| **Main Flow** | 1. Pricing Analyst mở yêu cầu cần thanh toán và kiểm tra bảng chi tiết chi phí.<br>2. System tổng hợp giá dịch vụ, thuế VAT và phí phục vụ.<br>3. Pricing Analyst chọn phương thức thanh toán (Ghi nợ phòng Folio / Tiền mặt / Thẻ ngân hàng).<br>4. System gửi lệnh tính phí sang Folio PMS hoặc Payment Gateway.<br>5. Payment Gateway trả về kết quả giao dịch thành công.<br>6. System cập nhật trạng thái đơn dịch vụ thành Paid và in hóa đơn thanh toán. |
| **Alternative Flow** | AF1 — Ghi nợ về tài khoản công ty: Pricing Analyst chọn nợ công ty, kiểm tra hạn mức nợ hợp lệ.<br>AF2 — Thanh toán bằng voucher: System trừ giá trị voucher trước khi tính số tiền còn lại. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Dynamic Pricing Status |
| **Actor(s)** | Primary: Pricing Analyst / Secondary: PMS Central Database |
...
| **Main Flow** | 1. Pricing Analyst mở danh sách công việc được phân công.<br>2. System hiển thị các yêu cầu đang thực hiện.<br>3. Pricing Analyst chọn yêu cầu và thay đổi trạng thái sang In-Progress hoặc Completed.<br>4. System ghi nhận thời điểm cập nhật và nhân viên thực hiện.<br>5. System đồng bộ trạng thái mới sang PMS Central Database.<br>6. System gửi thông báo trạng thái mới tới khách hàng. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Dynamic Pricing Request |
| **Actor(s)** | Primary: Revenue Director / Secondary: None |
...
| **Main Flow** | 1. Revenue Director truy cập danh sách yêu cầu chờ duyệt.<br>2. System hiển thị chi tiết nội dung, chi phí và lý do yêu cầu.<br>3. Revenue Director kiểm tra tính hợp lệ và hạn mức cho phép.<br>4. Revenue Director nhấn Phê duyệt (Approve).<br>5. System chuyển trạng thái yêu cầu sang Approved và chuyển tiếp cho nhân viên xử lý.<br>6. System lưu lại lịch sử phê duyệt của quản lý. |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Dynamic Pricing Report |
| **Actor(s)** | Primary: Revenue Director / Secondary: Audit & Compliance System |
...
| **Main Flow** | 1. Revenue Director chọn phân hệ Báo cáo và chọn khoảng thời gian (Ngày/Tuần/Tháng).<br>2. System truy vấn dữ liệu vận hành từ CSDL trung tâm.<br>3. System tính toán các chỉ số KPI, tổng doanh thu và biểu đồ xu hướng.<br>4. System xuất màn hình báo cáo tổng quan trực quan.<br>5. Revenue Director chọn Tải báo cáo dạng PDF/Excel.<br>6. System gửi bản sao báo cáo tới Audit & Compliance System. |
