# Use Case Diagram — Hotel Procurement & Inventory System
...
flowchart LR
    ...
    subgraph SystemBoundary[" Hotel Procurement & Inventory System "]
        UC01(["Register Procurement Request"])
        UC02(["Process Procurement Transaction"])
        UC03(["Update Procurement Status"])
        UC04(["Approve Procurement Request"])
        UC05(["Generate Procurement Report"])
        UC06(["Configure Procurement Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Procurement History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Procurement Task"])
        UC11(["Audit Procurement Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Procurement Request | Operations Staff | Hotel Guest | Create new operational request in Hotel Procurement & Inventory System. | High |
| 2 | UC02 | Process Procurement Transaction | Operations Staff | Payment Gateway | Execute payment or billing charge for Hotel Procurement & Inventory System. | High |
| 3 | UC03 | Update Procurement Status | Operations Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Procurement Request | Department Manager | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Procurement Report | Department Manager | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Procurement Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search Procurement History | Operations Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Procurement Task | Operations Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Procurement Logs | Department Manager | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Procurement Request |
| **Actor(s)** | Primary: Operations Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Hotel Procurement & Inventory System. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Procurement Transaction |
| **Actor(s)** | Primary: Operations Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Hotel Procurement & Inventory System. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Procurement Status |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Procurement Report |
