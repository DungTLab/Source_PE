# Use Case Diagram — Hotel Restaurant & F&B Management System
...
flowchart LR
    ...
    subgraph SystemBoundary[" Hotel Restaurant & F&B Management System "]
        UC01(["Register F&B Request"])
        UC02(["Process F&B Transaction"])
        UC03(["Update F&B Status"])
        UC04(["Approve F&B Request"])
        UC05(["Generate F&B Report"])
        UC06(["Configure F&B Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search F&B History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel F&B Task"])
        UC11(["Audit F&B Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register F&B Request | Order Runner | Hotel Guest | Create new operational request in Hotel Restaurant & F&B Management System. | High |
| 2 | UC02 | Process F&B Transaction | Order Runner | Payment Gateway | Execute payment or billing charge for Hotel Restaurant & F&B Management System. | High |
| 3 | UC03 | Update F&B Status | Order Runner | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve F&B Request | F&B Director | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate F&B Report | F&B Director | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure F&B Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
| 7 | UC07 | Send Service Notification | Order Runner | Notification Service | Dispatch automated alerts to guests or staff. | Medium |
| 8 | UC08 | Search F&B History | Order Runner | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel F&B Task | Order Runner | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit F&B Logs | F&B Director | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register F&B Request |
| **Actor(s)** | Primary: Order Runner / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Hotel Restaurant & F&B Management System. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process F&B Transaction |
| **Actor(s)** | Primary: Order Runner / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Hotel Restaurant & F&B Management System. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update F&B Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve F&B Request |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate F&B Report |
