# Use Case Diagram — Key Card & Access Control System
...
flowchart LR
    ...
    subgraph SystemBoundary[" Key Card & Access Control System "]
        UC01(["Register Access Control Request"])
        UC02(["Process Access Control Transaction"])
        UC03(["Update Access Control Status"])
        UC04(["Approve Access Control Request"])
        UC05(["Generate Access Control Report"])
        UC06(["Configure Access Control Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Access Control History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Access Control Task"])
        UC11(["Audit Access Control Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Access Control Request | Operations Staff | Hotel Guest | Create new operational request in Key Card & Access Control System. | High |
| 2 | UC02 | Process Access Control Transaction | Operations Staff | Payment Gateway | Execute payment or billing charge for Key Card & Access Control System. | High |
| 3 | UC03 | Update Access Control Status | Operations Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Access Control Request | Department Manager | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Access Control Report | Department Manager | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Access Control Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search Access Control History | Operations Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Access Control Task | Operations Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Access Control Logs | Department Manager | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Access Control Request |
| **Actor(s)** | Primary: Operations Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Key Card & Access Control System. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Access Control Transaction |
| **Actor(s)** | Primary: Operations Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Key Card & Access Control System. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Access Control Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Access Control Request |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Access Control Report |
