# Use Case Diagram — Leisure & Activities Booking System
...
flowchart LR
    ...
    subgraph SystemBoundary[" Leisure & Activities Booking System "]
        UC01(["Register Recreation Request"])
        UC02(["Process Recreation Transaction"])
        UC03(["Update Recreation Status"])
        UC04(["Approve Recreation Request"])
        UC05(["Generate Recreation Report"])
        UC06(["Configure Recreation Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Recreation History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Recreation Task"])
        UC11(["Audit Recreation Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Recreation Request | Operations Staff | Hotel Guest | Create new operational request in Leisure & Activities Booking System. | High |
| 2 | UC02 | Process Recreation Transaction | Operations Staff | Payment Gateway | Execute payment or billing charge for Leisure & Activities Booking System. | High |
| 3 | UC03 | Update Recreation Status | Operations Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Recreation Request | Department Manager | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Recreation Report | Department Manager | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Recreation Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search Recreation History | Operations Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Recreation Task | Operations Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Recreation Logs | Department Manager | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Recreation Request |
| **Actor(s)** | Primary: Operations Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Leisure & Activities Booking System. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Recreation Transaction |
| **Actor(s)** | Primary: Operations Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Leisure & Activities Booking System. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Recreation Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Recreation Request |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Recreation Report |
