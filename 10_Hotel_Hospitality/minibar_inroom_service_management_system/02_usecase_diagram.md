# Use Case Diagram — Mini-Bar & In-Room Service Management System

## Mermaid Code

```mermaid
flowchart LR
    actor_Order_Runner["Order Runner"]
    actor_Hotel_Guest["Hotel Guest"]
    actor_Payment_Gateway["Payment Gateway"]
    actor_PMS_Central_Database["PMS Central Database"]
    actor_F_B_Director["F&B Director"]
    ...
    subgraph SystemBoundary[" Mini-Bar & In-Room Service Management System "]
        UC01(["Register Room Service Request"])
        UC02(["Process Room Service Transaction"])
        UC03(["Update Room Service Status"])
        UC04(["Approve Room Service Request"])
        UC05(["Generate Room Service Report"])
        UC06(["Configure Room Service Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Room Service History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Room Service Task"])
        UC11(["Audit Room Service Logs"])
        UC12(["Export Performance Analytics"])
    end

    actor_Order_Runner --> UC01
    UC01 --> actor_Hotel_Guest
    actor_Order_Runner --> UC02
    UC02 --> actor_Payment_Gateway
    actor_Order_Runner --> UC03
    UC03 --> actor_PMS_Central_Database
    actor_F_B_Director --> UC04
    actor_F_B_Director --> UC05
    UC05 --> actor_Audit___Compliance_System
    actor_System_Administrator --> UC06
    actor_Order_Runner --> UC07
    UC07 --> actor_Notification_Service
    actor_Order_Runner --> UC08
    actor_PMS_Central_Database --> UC09
    actor_Order_Runner --> UC10
    UC10 --> actor_Hotel_Guest
    actor_F_B_Director --> UC11
    UC11 --> actor_Audit___Compliance_System
    actor_F_B_Director --> UC12
    UC01 -.->|"<<include>>"| UC02
    UC03 -.->|"<<extend>>"| UC04
```
...
|---|-------|------------|------------------|-------------------|
| 1 | Order Runner | Primary | Phụ trách các tác vụ liên quan đến Order Runner trong hệ thống | UC01, UC02, UC03, UC07, UC08, UC10 |
...
| 5 | F&B Director | Primary | Phụ trách các tác vụ liên quan đến F&B Director trong hệ thống | UC04, UC05, UC11, UC12 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Room Service Request | Order Runner | Hotel Guest | Create new operational request in Mini-Bar & In-Room Service Management System. | High |
| 2 | UC02 | Process Room Service Transaction | Order Runner | Payment Gateway | Execute payment or billing charge for Mini-Bar & In-Room Service Management System. | High |
| 3 | UC03 | Update Room Service Status | Order Runner | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Room Service Request | F&B Director | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Room Service Report | F&B Director | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Room Service Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
| 7 | UC07 | Send Service Notification | Order Runner | Notification Service | Dispatch automated alerts to guests or staff. | Medium |
| 8 | UC08 | Search Room Service History | Order Runner | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Room Service Task | Order Runner | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Room Service Logs | F&B Director | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
| 12 | UC12 | Export Performance Analytics | F&B Director | None | Export operational data to Excel or PDF format. | Low |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Room Service Request |
| **Actor(s)** | Primary: Order Runner / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Mini-Bar & In-Room Service Management System. |
| **Precondition** | 1. Người dùng đã đăng nhập hệ thống với quyền hạn hợp lệ.<br>2. Danh mục dịch vụ và khách hàng đã sẵn sàng. |
| **Main Flow** | 1. Order Runner truy cập màn hình tạo yêu cầu dịch vụ mới.<br>2. System hiển thị biểu mẫu nhập liệu chuẩn hóa.<br>3. Order Runner chọn thông tin Hotel Guest và chi tiết dịch vụ yêu cầu.<br>4. System kiểm tra tính hợp lệ của thông tin và tình trạng khả dụng.<br>5. Order Runner nhấn Xác nhận khởi tạo yêu cầu.<br>6. System tạo mới hồ sơ, lưu vào CSDL và gửi thông báo xác nhận. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Room Service Transaction |
| **Actor(s)** | Primary: Order Runner / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Mini-Bar & In-Room Service Management System. |
...
| **Main Flow** | 1. Order Runner mở yêu cầu cần thanh toán và kiểm tra bảng chi tiết chi phí.<br>2. System tổng hợp giá dịch vụ, thuế VAT và phí phục vụ.<br>3. Order Runner chọn phương thức thanh toán (Ghi nợ phòng Folio / Tiền mặt / Thẻ ngân hàng).<br>4. System gửi lệnh tính phí sang Folio PMS hoặc Payment Gateway.<br>5. Payment Gateway trả về kết quả giao dịch thành công.<br>6. System cập nhật trạng thái đơn dịch vụ thành Paid và in hóa đơn thanh toán. |
| **Alternative Flow** | AF1 — Ghi nợ về tài khoản công ty: Order Runner chọn nợ công ty, kiểm tra hạn mức nợ hợp lệ.<br>AF2 — Thanh toán bằng voucher: System trừ giá trị voucher trước khi tính số tiền còn lại. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Room Service Status |
| **Actor(s)** | Primary: Order Runner / Secondary: PMS Central Database |
...
| **Main Flow** | 1. Order Runner mở danh sách công việc được phân công.<br>2. System hiển thị các yêu cầu đang thực hiện.<br>3. Order Runner chọn yêu cầu và thay đổi trạng thái sang In-Progress hoặc Completed.<br>4. System ghi nhận thời điểm cập nhật và nhân viên thực hiện.<br>5. System đồng bộ trạng thái mới sang PMS Central Database.<br>6. System gửi thông báo trạng thái mới tới khách hàng. |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Room Service Request |
| **Actor(s)** | Primary: F&B Director / Secondary: None |
...
| **Main Flow** | 1. F&B Director truy cập danh sách yêu cầu chờ duyệt.<br>2. System hiển thị chi tiết nội dung, chi phí và lý do yêu cầu.<br>3. F&B Director kiểm tra tính hợp lệ và hạn mức cho phép.<br>4. F&B Director nhấn Phê duyệt (Approve).<br>5. System chuyển trạng thái yêu cầu sang Approved và chuyển tiếp cho nhân viên xử lý.<br>6. System lưu lại lịch sử phê duyệt của quản lý. |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Room Service Report |
| **Actor(s)** | Primary: F&B Director / Secondary: Audit & Compliance System |
...
| **Main Flow** | 1. F&B Director chọn phân hệ Báo cáo và chọn khoảng thời gian (Ngày/Tuần/Tháng).<br>2. System truy vấn dữ liệu vận hành từ CSDL trung tâm.<br>3. System tính toán các chỉ số KPI, tổng doanh thu và biểu đồ xu hướng.<br>4. System xuất màn hình báo cáo tổng quan trực quan.<br>5. F&B Director chọn Tải báo cáo dạng PDF/Excel.<br>6. System gửi bản sao báo cáo tới Audit & Compliance System. |
