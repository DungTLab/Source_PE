# Use Case Diagram — Motel & Budget Accommodation System
...
flowchart LR
    ...
    subgraph SystemBoundary[" Motel & Budget Accommodation System "]
        UC01(["Register Budget Request"])
        UC02(["Process Budget Transaction"])
        UC03(["Update Budget Status"])
        UC04(["Approve Budget Request"])
        UC05(["Generate Budget Report"])
        UC06(["Configure Budget Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Budget History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Budget Task"])
        UC11(["Audit Budget Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Budget Request | Operations Staff | Hotel Guest | Create new operational request in Motel & Budget Accommodation System. | High |
| 2 | UC02 | Process Budget Transaction | Operations Staff | Payment Gateway | Execute payment or billing charge for Motel & Budget Accommodation System. | High |
| 3 | UC03 | Update Budget Status | Operations Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Budget Request | Department Manager | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Budget Report | Department Manager | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Budget Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search Budget History | Operations Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Budget Task | Operations Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Budget Logs | Department Manager | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Budget Request |
| **Actor(s)** | Primary: Operations Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Motel & Budget Accommodation System. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Budget Transaction |
| **Actor(s)** | Primary: Operations Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Motel & Budget Accommodation System. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Budget Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Budget Request |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Budget Report |
