# Use Case Diagram — Night Audit System (Hotel)
...
flowchart LR
    ...
    subgraph SystemBoundary[" Night Audit System (Hotel) "]
        UC01(["Register Audit Request"])
        UC02(["Process Audit Transaction"])
        UC03(["Update Audit Status"])
        UC04(["Approve Audit Request"])
        UC05(["Generate Audit Report"])
        UC06(["Configure Audit Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Audit History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Audit Task"])
        UC11(["Audit Audit Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Audit Request | Operations Staff | Hotel Guest | Create new operational request in Night Audit System (Hotel). | High |
| 2 | UC02 | Process Audit Transaction | Operations Staff | Payment Gateway | Execute payment or billing charge for Night Audit System (Hotel). | High |
| 3 | UC03 | Update Audit Status | Operations Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Audit Request | Department Manager | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Audit Report | Department Manager | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Audit Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search Audit History | Operations Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Audit Task | Operations Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Audit Logs | Department Manager | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Audit Request |
| **Actor(s)** | Primary: Operations Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Night Audit System (Hotel). |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Audit Transaction |
| **Actor(s)** | Primary: Operations Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Night Audit System (Hotel). |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Audit Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Audit Request |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Audit Report |
