# Use Case Diagram — Resort & Vacation Package Management System
...
flowchart LR
    ...
    subgraph SystemBoundary[" Resort & Vacation Package Management System "]
        UC01(["Register Resort Request"])
        UC02(["Process Resort Transaction"])
        UC03(["Update Resort Status"])
        UC04(["Approve Resort Request"])
        UC05(["Generate Resort Report"])
        UC06(["Configure Resort Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Resort History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Resort Task"])
        UC11(["Audit Resort Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Resort Request | Operations Staff | Hotel Guest | Create new operational request in Resort & Vacation Package Management System. | High |
| 2 | UC02 | Process Resort Transaction | Operations Staff | Payment Gateway | Execute payment or billing charge for Resort & Vacation Package Management System. | High |
| 3 | UC03 | Update Resort Status | Operations Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Resort Request | Department Manager | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Resort Report | Department Manager | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Resort Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search Resort History | Operations Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Resort Task | Operations Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Resort Logs | Department Manager | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Resort Request |
| **Actor(s)** | Primary: Operations Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Resort & Vacation Package Management System. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Resort Transaction |
| **Actor(s)** | Primary: Operations Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Resort & Vacation Package Management System. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Resort Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Resort Request |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Resort Report |
