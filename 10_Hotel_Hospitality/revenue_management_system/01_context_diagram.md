# Context Diagram — Revenue Management System (Hotel)

## Mermaid Code

```mermaid
flowchart LR
    Actor1["Pricing Analyst"]
    Actor2["Revenue Director"]
    Actor3["Hotel Guest"]
    ...
    System((" Revenue Management System (Hotel) "))
...
|---|-------|------------|---------------------|---------------------------|-----------------|
| 1 | Pricing Analyst | Primary | Operation requests, status updates, transaction entries | Task assignments, system validations, operational logs | Nhân viên vận hành chính của Revenue Management System (Hotel) |
| 2 | Revenue Director | Primary | Approval requests, strategy configs, policy updates | Performance reports, exception alerts, operational dashboards | Quản lý cấp cao giám sát hoạt động của Revenue Management System (Hotel) |
...
Hệ thống Revenue Management System (Hotel) chịu trách nhiệm xử lý toàn bộ quy trình nghiệp vụ chuyên sâu thuộc lĩnh vực Revenue. Hệ thống quản lý dữ liệu đầu vào từ Pricing Analyst và Hotel Guest, đồng bộ hóa tài khoản và giao dịch với PMS Central Database, đồng thời ủy quyền xử lý thanh toán tài chính cho Payment Gateway và gửi thông báo qua Notification Service.
