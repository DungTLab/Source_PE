# Use Case Diagram — Revenue Management System (Hotel)
...
flowchart LR
    ...
    subgraph SystemBoundary[" Revenue Management System (Hotel) "]
        UC01(["Register Revenue Request"])
        UC02(["Process Revenue Transaction"])
        UC03(["Update Revenue Status"])
        UC04(["Approve Revenue Request"])
        UC05(["Generate Revenue Report"])
        UC06(["Configure Revenue Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Revenue History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Revenue Task"])
        UC11(["Audit Revenue Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Revenue Request | Pricing Analyst | Hotel Guest | Create new operational request in Revenue Management System (Hotel). | High |
| 2 | UC02 | Process Revenue Transaction | Pricing Analyst | Payment Gateway | Execute payment or billing charge for Revenue Management System (Hotel). | High |
| 3 | UC03 | Update Revenue Status | Pricing Analyst | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Revenue Request | Revenue Director | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Revenue Report | Revenue Director | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Revenue Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search Revenue History | Pricing Analyst | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Revenue Task | Pricing Analyst | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Revenue Logs | Revenue Director | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Revenue Request |
| **Actor(s)** | Primary: Pricing Analyst / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Revenue Management System (Hotel). |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Revenue Transaction |
| **Actor(s)** | Primary: Pricing Analyst / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Revenue Management System (Hotel). |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Revenue Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Revenue Request |
