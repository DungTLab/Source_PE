# Context Diagram — Room Service Order Tracking System
...
flowchart LR
    ...
    System((" Room Service Order Tracking System "))
...
|---|-------|------------|---------------------|---------------------------|-----------------|
| 1 | Order Runner | Primary | Operation requests, status updates, transaction entries | Task assignments, system validations, operational logs | Nhân viên vận hành chính của Room Service Order Tracking System |
| 2 | F&B Director | Primary | Approval requests, strategy configs, policy updates | Performance reports, exception alerts, operational dashboards | Quản lý cấp cao giám sát hoạt động của Room Service Order Tracking System |
...
Hệ thống Room Service Order Tracking System chịu trách nhiệm xử lý toàn bộ quy trình nghiệp vụ chuyên sâu thuộc lĩnh vực Room Service. Hệ thống quản lý dữ liệu đầu vào từ Order Runner và Hotel Guest, đồng bộ hóa tài khoản và giao dịch với PMS Central Database, đồng thời ủy quyền xử lý thanh toán tài chính cho Payment Gateway và gửi thông báo qua Notification Service.
