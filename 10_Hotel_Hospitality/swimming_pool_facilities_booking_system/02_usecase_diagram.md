# Use Case Diagram — Swimming Pool & Facilities Booking System
...
flowchart LR
    ...
    subgraph SystemBoundary[" Swimming Pool & Facilities Booking System "]
        UC01(["Register Facilities Request"])
        UC02(["Process Facilities Transaction"])
        UC03(["Update Facilities Status"])
        UC04(["Approve Facilities Request"])
        UC05(["Generate Facilities Report"])
        UC06(["Configure Facilities Settings"])
        UC07(["Send Service Notification"])
        UC08(["Search Facilities History"])
        UC09(["Synchronize PMS Data"])
        UC10(["Cancel Facilities Task"])
        UC11(["Audit Facilities Logs"])
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Facilities Request | Operations Staff | Hotel Guest | Create new operational request in Swimming Pool & Facilities Booking System. | High |
| 2 | UC02 | Process Facilities Transaction | Operations Staff | Payment Gateway | Execute payment or billing charge for Swimming Pool & Facilities Booking System. | High |
| 3 | UC03 | Update Facilities Status | Operations Staff | PMS Central Database | Modify operational status and update PMS database. | High |
| 4 | UC04 | Approve Facilities Request | Department Manager | None | Review and approve high-value or special requests. | High |
| 5 | UC05 | Generate Facilities Report | Department Manager | Audit & Compliance System | Compile performance metrics and operational summaries. | High |
| 6 | UC06 | Configure Facilities Settings | System Administrator | None | Maintain system master data, roles, and rules. | Medium |
...
| 8 | UC08 | Search Facilities History | Operations Staff | None | Query historical logs and previous records. | Medium |
| 9 | UC09 | Synchronize PMS Data | PMS Central Database | None | Sync real-time guest and room data with central PMS. | High |
| 10 | UC10 | Cancel Facilities Task | Operations Staff | Hotel Guest | Cancel active request upon guest cancellation. | Low |
| 11 | UC11 | Audit Facilities Logs | Department Manager | Audit & Compliance System | Review user audit trails and security compliance. | Medium |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Facilities Request |
| **Actor(s)** | Primary: Operations Staff / Secondary: Hotel Guest |
| **Description** | Thực hiện tiếp nhận và tạo mới yêu cầu nghiệp vụ trong hệ thống Swimming Pool & Facilities Booking System. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Process Facilities Transaction |
| **Actor(s)** | Primary: Operations Staff / Secondary: Payment Gateway |
| **Description** | Xử lý thanh toán hoặc ghi nợ tài khoản dịch vụ cho Swimming Pool & Facilities Booking System. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Update Facilities Status |
...
|-------|--------|
| **UC ID** | UC04 |
| **Use Case Name** | Approve Facilities Request |
...
|-------|--------|
| **UC ID** | UC05 |
| **Use Case Name** | Generate Facilities Report |
