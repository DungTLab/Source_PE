# Context Diagram — VIP Guest Management System

## Mermaid Code

```mermaid
flowchart LR
    Actor1["Personal Butler"]
    Actor2["VIP Relations Director"]
    Actor3["Hotel Guest"]
    ...
    System((" VIP Guest Management System "))
...
|---|-------|------------|---------------------|---------------------------|-----------------|
| 1 | Personal Butler | Primary | Operation requests, status updates, transaction entries | Task assignments, system validations, operational logs | Nhân viên vận hành chính của VIP Guest Management System |
| 2 | VIP Relations Director | Primary | Approval requests, strategy configs, policy updates | Performance reports, exception alerts, operational dashboards | Quản lý cấp cao giám sát hoạt động của VIP Guest Management System |
...
Hệ thống VIP Guest Management System chịu trách nhiệm xử lý toàn bộ quy trình nghiệp vụ chuyên sâu thuộc lĩnh vực VIP. Hệ thống quản lý dữ liệu đầu vào từ Personal Butler và Hotel Guest, đồng bộ hóa tài khoản và giao dịch với PMS Central Database, đồng thời ủy quyền xử lý thanh toán tài chính cho Payment Gateway và gửi thông báo qua Notification Service.
