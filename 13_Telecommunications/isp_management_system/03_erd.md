# Conceptual ERD — ISP Management System

## Mermaid Code

```mermaid
erDiagram
    ISPMANAGEMENTSYSTEM_ELEMENT {
        element_id PK
        element_name string
    ...
    ISPMANAGEMENTSYSTEM_ELEMENT ||--o{ OPERATIONAL_SESSION : "handles"
    ISPMANAGEMENTSYSTEM_ELEMENT ||--o{ FAULT_ALARM : "triggers"
    ISPMANAGEMENTSYSTEM_ELEMENT ||--o{ MAINTENANCE_TICKET : "requires"
    ISPMANAGEMENTSYSTEM_ELEMENT ||--o{ PERFORMANCE_METRIC : "emits"
    CONFIGURATION_POLICY ||--o{ ISPMANAGEMENTSYSTEM_ELEMENT : "governs"
...
|---|-------------|-----------------|-------------|----------------|-------------------|
| 1 | ISPMANAGEMENTSYSTEM_ELEMENT | Phần tử Hệ Thống Quản Lý Nhà Cung Cấp Dịch Vụ Internet (ISP) | Main operational unit in ISP Management System | element_id PK, element_name string, serial_number string | handles, triggers, requires, emits, governs |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | ISPMANAGEMENTSYSTEM_ELEMENT | one-to-many | OPERATIONAL_SESSION | handles | One element handles multiple operational sessions |
| 2 | ISPMANAGEMENTSYSTEM_ELEMENT | one-to-many | FAULT_ALARM | triggers | One element triggers multiple fault alarms |
| 3 | ISPMANAGEMENTSYSTEM_ELEMENT | one-to-many | MAINTENANCE_TICKET | requires | One element may require multiple maintenance tickets |
| 4 | ISPMANAGEMENTSYSTEM_ELEMENT | one-to-many | PERFORMANCE_METRIC | emits | One element emits time-series performance metrics |
| 5 | CONFIGURATION_POLICY | one-to-many | ISPMANAGEMENTSYSTEM_ELEMENT | governs | One policy governs multiple network elements |
