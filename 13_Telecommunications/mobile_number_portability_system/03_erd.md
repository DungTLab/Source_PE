# Conceptual ERD — Mobile Number Portability System

## Mermaid Code

```mermaid
erDiagram
    MOBILENUMBERPORTABILITYSYSTEM_ELEMENT {
        element_id PK
        element_name string
    ...
    MOBILENUMBERPORTABILITYSYSTEM_ELEMENT ||--o{ OPERATIONAL_SESSION : "handles"
    MOBILENUMBERPORTABILITYSYSTEM_ELEMENT ||--o{ FAULT_ALARM : "triggers"
    MOBILENUMBERPORTABILITYSYSTEM_ELEMENT ||--o{ MAINTENANCE_TICKET : "requires"
    MOBILENUMBERPORTABILITYSYSTEM_ELEMENT ||--o{ PERFORMANCE_METRIC : "emits"
    CONFIGURATION_POLICY ||--o{ MOBILENUMBERPORTABILITYSYSTEM_ELEMENT : "governs"
...
|---|-------------|-----------------|-------------|----------------|-------------------|
| 1 | MOBILENUMBERPORTABILITYSYSTEM_ELEMENT | Phần tử Hệ Thống Chuyển Mạng Giữ Số Di Động | Main operational unit in Mobile Number Portability System | element_id PK, element_name string, serial_number string | handles, triggers, requires, emits, governs |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | MOBILENUMBERPORTABILITYSYSTEM_ELEMENT | one-to-many | OPERATIONAL_SESSION | handles | One element handles multiple operational sessions |
| 2 | MOBILENUMBERPORTABILITYSYSTEM_ELEMENT | one-to-many | FAULT_ALARM | triggers | One element triggers multiple fault alarms |
| 3 | MOBILENUMBERPORTABILITYSYSTEM_ELEMENT | one-to-many | MAINTENANCE_TICKET | requires | One element may require multiple maintenance tickets |
| 4 | MOBILENUMBERPORTABILITYSYSTEM_ELEMENT | one-to-many | PERFORMANCE_METRIC | emits | One element emits time-series performance metrics |
| 5 | CONFIGURATION_POLICY | one-to-many | MOBILENUMBERPORTABILITYSYSTEM_ELEMENT | governs | One policy governs multiple network elements |
