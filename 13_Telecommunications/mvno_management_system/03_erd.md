# Conceptual ERD — MVNO Management System

## Mermaid Code

```mermaid
erDiagram
    MVNOMANAGEMENTSYSTEM_ELEMENT {
        element_id PK
        element_name string
    ...
    MVNOMANAGEMENTSYSTEM_ELEMENT ||--o{ OPERATIONAL_SESSION : "handles"
    MVNOMANAGEMENTSYSTEM_ELEMENT ||--o{ FAULT_ALARM : "triggers"
    MVNOMANAGEMENTSYSTEM_ELEMENT ||--o{ MAINTENANCE_TICKET : "requires"
    MVNOMANAGEMENTSYSTEM_ELEMENT ||--o{ PERFORMANCE_METRIC : "emits"
    CONFIGURATION_POLICY ||--o{ MVNOMANAGEMENTSYSTEM_ELEMENT : "governs"
...
|---|-------------|-----------------|-------------|----------------|-------------------|
| 1 | MVNOMANAGEMENTSYSTEM_ELEMENT | Phần tử Hệ Thống Quản Lý Mạng Di Động Ảo (MVNO) | Main operational unit in MVNO Management System | element_id PK, element_name string, serial_number string | handles, triggers, requires, emits, governs |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | MVNOMANAGEMENTSYSTEM_ELEMENT | one-to-many | OPERATIONAL_SESSION | handles | One element handles multiple operational sessions |
| 2 | MVNOMANAGEMENTSYSTEM_ELEMENT | one-to-many | FAULT_ALARM | triggers | One element triggers multiple fault alarms |
| 3 | MVNOMANAGEMENTSYSTEM_ELEMENT | one-to-many | MAINTENANCE_TICKET | requires | One element may require multiple maintenance tickets |
| 4 | MVNOMANAGEMENTSYSTEM_ELEMENT | one-to-many | PERFORMANCE_METRIC | emits | One element emits time-series performance metrics |
| 5 | CONFIGURATION_POLICY | one-to-many | MVNOMANAGEMENTSYSTEM_ELEMENT | governs | One policy governs multiple network elements |
