# Conceptual ERD — Roaming Management System

## Mermaid Code

```mermaid
erDiagram
    ROAMINGMANAGEMENTSYSTEM_ELEMENT {
        element_id PK
        element_name string
    ...
    ROAMINGMANAGEMENTSYSTEM_ELEMENT ||--o{ OPERATIONAL_SESSION : "handles"
    ROAMINGMANAGEMENTSYSTEM_ELEMENT ||--o{ FAULT_ALARM : "triggers"
    ROAMINGMANAGEMENTSYSTEM_ELEMENT ||--o{ MAINTENANCE_TICKET : "requires"
    ROAMINGMANAGEMENTSYSTEM_ELEMENT ||--o{ PERFORMANCE_METRIC : "emits"
    CONFIGURATION_POLICY ||--o{ ROAMINGMANAGEMENTSYSTEM_ELEMENT : "governs"
...
|---|-------------|-----------------|-------------|----------------|-------------------|
| 1 | ROAMINGMANAGEMENTSYSTEM_ELEMENT | Phần tử Hệ Thống Quản Lý Chuyển Vùng Cước Gọi (Roaming) | Main operational unit in Roaming Management System | element_id PK, element_name string, serial_number string | handles, triggers, requires, emits, governs |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | ROAMINGMANAGEMENTSYSTEM_ELEMENT | one-to-many | OPERATIONAL_SESSION | handles | One element handles multiple operational sessions |
| 2 | ROAMINGMANAGEMENTSYSTEM_ELEMENT | one-to-many | FAULT_ALARM | triggers | One element triggers multiple fault alarms |
| 3 | ROAMINGMANAGEMENTSYSTEM_ELEMENT | one-to-many | MAINTENANCE_TICKET | requires | One element may require multiple maintenance tickets |
| 4 | ROAMINGMANAGEMENTSYSTEM_ELEMENT | one-to-many | PERFORMANCE_METRIC | emits | One element emits time-series performance metrics |
| 5 | CONFIGURATION_POLICY | one-to-many | ROAMINGMANAGEMENTSYSTEM_ELEMENT | governs | One policy governs multiple network elements |
