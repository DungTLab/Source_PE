# Conceptual ERD — Service Provisioning System

## Mermaid Code

```mermaid
erDiagram
    SERVICEPROVISIONINGSYSTEM_ELEMENT {
        element_id PK
        element_name string
    ...
    SERVICEPROVISIONINGSYSTEM_ELEMENT ||--o{ OPERATIONAL_SESSION : "handles"
    SERVICEPROVISIONINGSYSTEM_ELEMENT ||--o{ FAULT_ALARM : "triggers"
    SERVICEPROVISIONINGSYSTEM_ELEMENT ||--o{ MAINTENANCE_TICKET : "requires"
    SERVICEPROVISIONINGSYSTEM_ELEMENT ||--o{ PERFORMANCE_METRIC : "emits"
    CONFIGURATION_POLICY ||--o{ SERVICEPROVISIONINGSYSTEM_ELEMENT : "governs"
...
|---|-------------|-----------------|-------------|----------------|-------------------|
| 1 | SERVICEPROVISIONINGSYSTEM_ELEMENT | Phần tử Hệ Thống Cung Cấp và Kích Hoạt Dịch Vụ | Main operational unit in Service Provisioning System | element_id PK, element_name string, serial_number string | handles, triggers, requires, emits, governs |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | SERVICEPROVISIONINGSYSTEM_ELEMENT | one-to-many | OPERATIONAL_SESSION | handles | One element handles multiple operational sessions |
| 2 | SERVICEPROVISIONINGSYSTEM_ELEMENT | one-to-many | FAULT_ALARM | triggers | One element triggers multiple fault alarms |
| 3 | SERVICEPROVISIONINGSYSTEM_ELEMENT | one-to-many | MAINTENANCE_TICKET | requires | One element may require multiple maintenance tickets |
| 4 | SERVICEPROVISIONINGSYSTEM_ELEMENT | one-to-many | PERFORMANCE_METRIC | emits | One element emits time-series performance metrics |
| 5 | CONFIGURATION_POLICY | one-to-many | SERVICEPROVISIONINGSYSTEM_ELEMENT | governs | One policy governs multiple network elements |
