# Conceptual ERD — SIM Card Management System

## Mermaid Code

```mermaid
erDiagram
    SIMCARDMANAGEMENTSYSTEM_ELEMENT {
        element_id PK
        element_name string
    ...
    SIMCARDMANAGEMENTSYSTEM_ELEMENT ||--o{ OPERATIONAL_SESSION : "handles"
    SIMCARDMANAGEMENTSYSTEM_ELEMENT ||--o{ FAULT_ALARM : "triggers"
    SIMCARDMANAGEMENTSYSTEM_ELEMENT ||--o{ MAINTENANCE_TICKET : "requires"
    SIMCARDMANAGEMENTSYSTEM_ELEMENT ||--o{ PERFORMANCE_METRIC : "emits"
    CONFIGURATION_POLICY ||--o{ SIMCARDMANAGEMENTSYSTEM_ELEMENT : "governs"
...
|---|-------------|-----------------|-------------|----------------|-------------------|
| 1 | SIMCARDMANAGEMENTSYSTEM_ELEMENT | Phần tử Hệ Thống Quản Lý Thẻ SIM | Main operational unit in SIM Card Management System | element_id PK, element_name string, serial_number string | handles, triggers, requires, emits, governs |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | SIMCARDMANAGEMENTSYSTEM_ELEMENT | one-to-many | OPERATIONAL_SESSION | handles | One element handles multiple operational sessions |
| 2 | SIMCARDMANAGEMENTSYSTEM_ELEMENT | one-to-many | FAULT_ALARM | triggers | One element triggers multiple fault alarms |
| 3 | SIMCARDMANAGEMENTSYSTEM_ELEMENT | one-to-many | MAINTENANCE_TICKET | requires | One element may require multiple maintenance tickets |
| 4 | SIMCARDMANAGEMENTSYSTEM_ELEMENT | one-to-many | PERFORMANCE_METRIC | emits | One element emits time-series performance metrics |
| 5 | CONFIGURATION_POLICY | one-to-many | SIMCARDMANAGEMENTSYSTEM_ELEMENT | governs | One policy governs multiple network elements |
