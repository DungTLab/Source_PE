# Conceptual ERD — Telecom Analytics Platform

## Mermaid Code

```mermaid
erDiagram
    TELECOMANALYTICSPLATFORM_ELEMENT {
        element_id PK
        element_name string
    ...
    TELECOMANALYTICSPLATFORM_ELEMENT ||--o{ OPERATIONAL_SESSION : "handles"
    TELECOMANALYTICSPLATFORM_ELEMENT ||--o{ FAULT_ALARM : "triggers"
    TELECOMANALYTICSPLATFORM_ELEMENT ||--o{ MAINTENANCE_TICKET : "requires"
    TELECOMANALYTICSPLATFORM_ELEMENT ||--o{ PERFORMANCE_METRIC : "emits"
    CONFIGURATION_POLICY ||--o{ TELECOMANALYTICSPLATFORM_ELEMENT : "governs"
...
|---|-------------|-----------------|-------------|----------------|-------------------|
| 1 | TELECOMANALYTICSPLATFORM_ELEMENT | Phần tử Nền Tảng Phân Tích Dữ Liệu Viễn Thông | Main operational unit in Telecom Analytics Platform | element_id PK, element_name string, serial_number string | handles, triggers, requires, emits, governs |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | TELECOMANALYTICSPLATFORM_ELEMENT | one-to-many | OPERATIONAL_SESSION | handles | One element handles multiple operational sessions |
| 2 | TELECOMANALYTICSPLATFORM_ELEMENT | one-to-many | FAULT_ALARM | triggers | One element triggers multiple fault alarms |
| 3 | TELECOMANALYTICSPLATFORM_ELEMENT | one-to-many | MAINTENANCE_TICKET | requires | One element may require multiple maintenance tickets |
| 4 | TELECOMANALYTICSPLATFORM_ELEMENT | one-to-many | PERFORMANCE_METRIC | emits | One element emits time-series performance metrics |
| 5 | CONFIGURATION_POLICY | one-to-many | TELECOMANALYTICSPLATFORM_ELEMENT | governs | One policy governs multiple network elements |
