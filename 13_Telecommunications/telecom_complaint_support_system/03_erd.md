# Conceptual ERD — Telecom Complaint & Support System

## Mermaid Code

```mermaid
erDiagram
    TELECOMCOMPLAINTSUPPORTSYSTEM_ELEMENT {
        element_id PK
        element_name string
    ...
    TELECOMCOMPLAINTSUPPORTSYSTEM_ELEMENT ||--o{ OPERATIONAL_SESSION : "handles"
    TELECOMCOMPLAINTSUPPORTSYSTEM_ELEMENT ||--o{ FAULT_ALARM : "triggers"
    TELECOMCOMPLAINTSUPPORTSYSTEM_ELEMENT ||--o{ MAINTENANCE_TICKET : "requires"
    TELECOMCOMPLAINTSUPPORTSYSTEM_ELEMENT ||--o{ PERFORMANCE_METRIC : "emits"
    CONFIGURATION_POLICY ||--o{ TELECOMCOMPLAINTSUPPORTSYSTEM_ELEMENT : "governs"
...
|---|-------------|-----------------|-------------|----------------|-------------------|
| 1 | TELECOMCOMPLAINTSUPPORTSYSTEM_ELEMENT | Phần tử Hệ Thống Hỗ Trợ và Tiếp Nhận Khiếu Nại Viễn Thông | Main operational unit in Telecom Complaint & Support System | element_id PK, element_name string, serial_number string | handles, triggers, requires, emits, governs |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | TELECOMCOMPLAINTSUPPORTSYSTEM_ELEMENT | one-to-many | OPERATIONAL_SESSION | handles | One element handles multiple operational sessions |
| 2 | TELECOMCOMPLAINTSUPPORTSYSTEM_ELEMENT | one-to-many | FAULT_ALARM | triggers | One element triggers multiple fault alarms |
| 3 | TELECOMCOMPLAINTSUPPORTSYSTEM_ELEMENT | one-to-many | MAINTENANCE_TICKET | requires | One element may require multiple maintenance tickets |
| 4 | TELECOMCOMPLAINTSUPPORTSYSTEM_ELEMENT | one-to-many | PERFORMANCE_METRIC | emits | One element emits time-series performance metrics |
| 5 | CONFIGURATION_POLICY | one-to-many | TELECOMCOMPLAINTSUPPORTSYSTEM_ELEMENT | governs | One policy governs multiple network elements |
