# Conceptual ERD — Telecom CRM System

## Mermaid Code

```mermaid
erDiagram
    TELECOMCRMSYSTEM_ELEMENT {
        element_id PK
        element_name string
    ...
    TELECOMCRMSYSTEM_ELEMENT ||--o{ OPERATIONAL_SESSION : "handles"
    TELECOMCRMSYSTEM_ELEMENT ||--o{ FAULT_ALARM : "triggers"
    TELECOMCRMSYSTEM_ELEMENT ||--o{ MAINTENANCE_TICKET : "requires"
    TELECOMCRMSYSTEM_ELEMENT ||--o{ PERFORMANCE_METRIC : "emits"
    CONFIGURATION_POLICY ||--o{ TELECOMCRMSYSTEM_ELEMENT : "governs"
...
|---|-------------|-----------------|-------------|----------------|-------------------|
| 1 | TELECOMCRMSYSTEM_ELEMENT | Phần tử Hệ Thống CRM Viễn Thông | Main operational unit in Telecom CRM System | element_id PK, element_name string, serial_number string | handles, triggers, requires, emits, governs |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | TELECOMCRMSYSTEM_ELEMENT | one-to-many | OPERATIONAL_SESSION | handles | One element handles multiple operational sessions |
| 2 | TELECOMCRMSYSTEM_ELEMENT | one-to-many | FAULT_ALARM | triggers | One element triggers multiple fault alarms |
| 3 | TELECOMCRMSYSTEM_ELEMENT | one-to-many | MAINTENANCE_TICKET | requires | One element may require multiple maintenance tickets |
| 4 | TELECOMCRMSYSTEM_ELEMENT | one-to-many | PERFORMANCE_METRIC | emits | One element emits time-series performance metrics |
| 5 | CONFIGURATION_POLICY | one-to-many | TELECOMCRMSYSTEM_ELEMENT | governs | One policy governs multiple network elements |
