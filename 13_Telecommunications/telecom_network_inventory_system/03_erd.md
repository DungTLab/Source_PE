# Conceptual ERD — Telecom Network Inventory System

## Mermaid Code

```mermaid
erDiagram
    TELECOMNETWORKINVENTORYSYSTEM_ELEMENT {
        element_id PK
        element_name string
    ...
    TELECOMNETWORKINVENTORYSYSTEM_ELEMENT ||--o{ OPERATIONAL_SESSION : "handles"
    TELECOMNETWORKINVENTORYSYSTEM_ELEMENT ||--o{ FAULT_ALARM : "triggers"
    TELECOMNETWORKINVENTORYSYSTEM_ELEMENT ||--o{ MAINTENANCE_TICKET : "requires"
    TELECOMNETWORKINVENTORYSYSTEM_ELEMENT ||--o{ PERFORMANCE_METRIC : "emits"
    CONFIGURATION_POLICY ||--o{ TELECOMNETWORKINVENTORYSYSTEM_ELEMENT : "governs"
...
|---|-------------|-----------------|-------------|----------------|-------------------|
| 1 | TELECOMNETWORKINVENTORYSYSTEM_ELEMENT | Phần tử Hệ Thống Quản Lý Thiết Bị và Hạ Tầng Mạng Viễn Thông | Main operational unit in Telecom Network Inventory System | element_id PK, element_name string, serial_number string | handles, triggers, requires, emits, governs |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | TELECOMNETWORKINVENTORYSYSTEM_ELEMENT | one-to-many | OPERATIONAL_SESSION | handles | One element handles multiple operational sessions |
| 2 | TELECOMNETWORKINVENTORYSYSTEM_ELEMENT | one-to-many | FAULT_ALARM | triggers | One element triggers multiple fault alarms |
| 3 | TELECOMNETWORKINVENTORYSYSTEM_ELEMENT | one-to-many | MAINTENANCE_TICKET | requires | One element may require multiple maintenance tickets |
| 4 | TELECOMNETWORKINVENTORYSYSTEM_ELEMENT | one-to-many | PERFORMANCE_METRIC | emits | One element emits time-series performance metrics |
| 5 | CONFIGURATION_POLICY | one-to-many | TELECOMNETWORKINVENTORYSYSTEM_ELEMENT | governs | One policy governs multiple network elements |
