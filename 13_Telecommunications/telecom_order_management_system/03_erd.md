# Conceptual ERD — Telecom Order Management System

## Mermaid Code

```mermaid
erDiagram
    TELECOMORDERMANAGEMENTSYSTEM_ELEMENT {
        element_id PK
        element_name string
    ...
    TELECOMORDERMANAGEMENTSYSTEM_ELEMENT ||--o{ OPERATIONAL_SESSION : "handles"
    TELECOMORDERMANAGEMENTSYSTEM_ELEMENT ||--o{ FAULT_ALARM : "triggers"
    TELECOMORDERMANAGEMENTSYSTEM_ELEMENT ||--o{ MAINTENANCE_TICKET : "requires"
    TELECOMORDERMANAGEMENTSYSTEM_ELEMENT ||--o{ PERFORMANCE_METRIC : "emits"
    CONFIGURATION_POLICY ||--o{ TELECOMORDERMANAGEMENTSYSTEM_ELEMENT : "governs"
...
|---|-------------|-----------------|-------------|----------------|-------------------|
| 1 | TELECOMORDERMANAGEMENTSYSTEM_ELEMENT | Phần tử Hệ Thống Quản Lý Đơn Hàng Viễn Thông | Main operational unit in Telecom Order Management System | element_id PK, element_name string, serial_number string | handles, triggers, requires, emits, governs |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | TELECOMORDERMANAGEMENTSYSTEM_ELEMENT | one-to-many | OPERATIONAL_SESSION | handles | One element handles multiple operational sessions |
| 2 | TELECOMORDERMANAGEMENTSYSTEM_ELEMENT | one-to-many | FAULT_ALARM | triggers | One element triggers multiple fault alarms |
| 3 | TELECOMORDERMANAGEMENTSYSTEM_ELEMENT | one-to-many | MAINTENANCE_TICKET | requires | One element may require multiple maintenance tickets |
| 4 | TELECOMORDERMANAGEMENTSYSTEM_ELEMENT | one-to-many | PERFORMANCE_METRIC | emits | One element emits time-series performance metrics |
| 5 | CONFIGURATION_POLICY | one-to-many | TELECOMORDERMANAGEMENTSYSTEM_ELEMENT | governs | One policy governs multiple network elements |
