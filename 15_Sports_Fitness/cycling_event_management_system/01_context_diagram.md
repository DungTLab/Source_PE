# Context Diagram — Cycling Event Management System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Cycling Event User"]
    DomainAdmin["Cycling Event Administrator"]
    ...
    System(("Cycling Event Management System"))

    PrimaryUser -->|"Submits cycling event requests"| System
    System -->|"Receives cycling event status"| PrimaryUser
    DomainAdmin -->|"Configures cycling event settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Cycling Event User | Primary | Submits cycling event requests, updates profile | Receives cycling event status, confirmation | Primary domain user |
| 2 | Cycling Event Administrator | Primary | Configures cycling event settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Cycling Event Management System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Quản Lý Sự Kiện Đua Xe Đạp. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
