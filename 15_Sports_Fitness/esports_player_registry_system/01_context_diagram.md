# Context Diagram — Esports Player Registry System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Esports Player Registry User"]
    DomainAdmin["Esports Player Registry Administrator"]
    ...
    System(("Esports Player Registry System"))

    PrimaryUser -->|"Submits esports player registry requests"| System
    System -->|"Receives esports player registry status"| PrimaryUser
    DomainAdmin -->|"Configures esports player registry settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Esports Player Registry User | Primary | Submits esports player registry requests, updates profile | Receives esports player registry status, confirmation | Primary domain user |
| 2 | Esports Player Registry Administrator | Primary | Configures esports player registry settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Esports Player Registry System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Đăng Ký Game Thủ Esports. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
