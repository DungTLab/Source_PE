# Use Case Diagram — Golf Course Management System

## Mermaid Code

```mermaid
flowchart LR
    actor_PrimaryUser["Golf Course User"]
    actor_DomainAdmin["Golf Course Administrator"]
    ...
    subgraph SystemBoundary["Golf Course Management System"]
        UC01(["Register Golf Course Account"])
        UC02(["Manage Golf Course Configuration"])
        UC03(["Schedule Golf Course Event"])
...
|---|-------|------------|------------------|-------------------|
| 1 | Golf Course User | Primary | Primary domain user | UC01, UC04, UC13 |
| 2 | Golf Course Administrator | Primary | System administrator | UC02, UC08, UC10, UC14 |
...
|---|-------|---------------|---------------|-----------------|-------------|----------|
| 1 | UC01 | Register Golf Course Account | Golf Course User | Payment Gateway API | Allows users to register and setup golf course profiles. | High |
| 2 | UC02 | Manage Golf Course Configuration | Golf Course Administrator | None | Configures core parameters and rules for golf course. | High |
| 3 | UC03 | Schedule Golf Course Event | Operational Manager | Field Official / Inspector | Schedules timelines, matches, or operational events. | High |
| 4 | UC04 | Process Payment Transaction | Golf Course User | Payment Gateway API | Executes online fee payments, deposits, or subscriptions. | High |
...
| 6 | UC06 | Send Automated Notifications | Notification & SMS Gateway | Golf Course User | Broadcasts system updates, schedule changes, and reminders. | Low |
| 7 | UC07 | Ingest IoT Sensor Telemetry | IoT Telemetry Gateway | Golf Course Administrator | Processes stream data from sensors, wearables, or gate scanners. | Medium |
| 8 | UC08 | Generate Compliance Audit | Golf Course Administrator | Sports Federation Audit Portal | Exports standardized regulatory compliance reports. | High |
| 9 | UC09 | Verify User Eligibility | Operational Manager | Golf Course User | Checks credentials, medical clearances, or membership status. | Medium |
| 10 | UC10 | Export Performance Analytics | Golf Course Administrator | None | Generates statistical trends, revenue charts, and operational dashboards. | High |
| 11 | UC11 | Manage Resource Allocation | Operational Manager | None | Assigns venues, equipment, or staff to active events. | Medium |
| 12 | UC12 | Handle Incident Report | Field Official / Inspector | Golf Course Administrator | Logs safety infractions, equipment failures, or participant disputes. | Medium |
| 13 | UC13 | Update User Profile | Golf Course User | None | Updates contact info, preferences, and emergency details. | Low |
| 14 | UC14 | Archive Historical Records | Golf Course Administrator | Sports Federation Audit Portal | Stores completed event logs and participant history for long-term audit. | Low |
...
|-------|--------|
| **UC ID** | UC01 |
| **Use Case Name** | Register Golf Course Account |
| **Actor(s)** | Primary: Golf Course User / Secondary: Payment Gateway API |
| **Description** | Allows users to register and setup golf course profiles. |
...
|-------|--------|
| **UC ID** | UC02 |
| **Use Case Name** | Manage Golf Course Configuration |
| **Actor(s)** | Primary: Golf Course Administrator / Secondary: None |
| **Description** | Configures core parameters and rules for golf course. |
| **Precondition** | 1. User is authenticated with appropriate role permissions.<br>2. System network connection and target database service are fully active. |
| **Main Flow** | 1. Golf Course Administrator initiates Manage Golf Course Configuration request via the system dashboard.<br>2. System validates input data parameters and displays confirmation screen.<br>3. Golf Course Administrator reviews details and submits final transaction.<br>4. System processes payload and communicates with None.<br>5. None returns authorization code and transaction status.<br>6. System updates internal record and returns success notification to Golf Course Administrator. |
| **Alternative Flow** | **AF1** — Saved Draft Flow: If user chooses save draft, system stores state without submitting.<br>**AF2** — Fast-track Flow: If user holds VIP badge, system bypasses standard queue validation. |
| **Exception Flow** | **EX1** — Network Timeout: If secondary system fails to respond within 10 seconds, system displays retry prompt.<br>**EX2** — Validation Error: If input fields contain invalid format, system highlights error fields. |
| **Postcondition** | Target transaction state is saved into DB and confirmation log is recorded. |
| **Business Rule** | **BR1**: All transactions must be encrypted using AES-256.<br>**BR2**: Logs must be archived for audit compliance. |
...
|-------|--------|
| **UC ID** | UC03 |
| **Use Case Name** | Schedule Golf Course Event |
...
| **Precondition** | 1. User is authenticated with appropriate role permissions.<br>2. System network connection and target database service are fully active. |
| **Main Flow** | 1. Golf Course User initiates Process Payment Transaction request via the system dashboard.<br>2. System validates input data parameters and displays confirmation screen.<br>3. Golf Course User reviews details and submits final transaction.<br>4. System processes payload and communicates with Payment Gateway API.<br>5. Payment Gateway API returns authorization code and transaction status.<br>6. System updates internal record and returns success notification to Golf Course User. |
| **Alternative Flow** | **AF1** — Saved Draft Flow: If user chooses save draft, system stores state without submitting.<br>**AF2** — Fast-track Flow: If user holds VIP badge, system bypasses standard queue validation. |
| **Exception Flow** | **EX1** — Network Timeout: If secondary system fails to respond within 10 seconds, system displays retry prompt.<br>**EX2** — Validation Error: If input fields contain invalid format, system highlights error fields. |
| **Postcondition** | Target transaction state is saved into DB and confirmation log is recorded. |
| **Business Rule** | **BR1**: All transactions must be encrypted using AES-256.<br>**BR2**: Logs must be archived for audit compliance. |
