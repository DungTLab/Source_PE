# Context Diagram — Indoor Sports Complex Management System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Indoor Sports Complex User"]
    DomainAdmin["Indoor Sports Complex Administrator"]
    ...
    System(("Indoor Sports Complex Management System"))

    PrimaryUser -->|"Submits indoor sports complex requests"| System
    System -->|"Receives indoor sports complex status"| PrimaryUser
    DomainAdmin -->|"Configures indoor sports complex settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Indoor Sports Complex User | Primary | Submits indoor sports complex requests, updates profile | Receives indoor sports complex status, confirmation | Primary domain user |
| 2 | Indoor Sports Complex Administrator | Primary | Configures indoor sports complex settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Indoor Sports Complex Management System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Quản Lý Khu Phức Hợp Thể Thao Trong Nhà. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
