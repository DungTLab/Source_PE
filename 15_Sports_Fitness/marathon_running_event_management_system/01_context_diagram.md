# Context Diagram — Marathon & Running Event Management System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Marathon & Running Event User"]
    DomainAdmin["Marathon & Running Event Administrator"]
    ...
    System(("Marathon & Running Event Management System"))

    PrimaryUser -->|"Submits marathon & running event requests"| System
    System -->|"Receives marathon & running event status"| PrimaryUser
    DomainAdmin -->|"Configures marathon & running event settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Marathon & Running Event User | Primary | Submits marathon & running event requests, updates profile | Receives marathon & running event status, confirmation | Primary domain user |
| 2 | Marathon & Running Event Administrator | Primary | Configures marathon & running event settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Marathon & Running Event Management System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Quản Lý Giải Chạy Marathon và Sự Kiện Chạy. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
