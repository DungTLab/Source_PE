# Context Diagram — Online Personal Training System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Online Personal Training User"]
    DomainAdmin["Online Personal Training Administrator"]
    ...
    System(("Online Personal Training System"))

    PrimaryUser -->|"Submits online personal training requests"| System
    System -->|"Receives online personal training status"| PrimaryUser
    DomainAdmin -->|"Configures online personal training settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Online Personal Training User | Primary | Submits online personal training requests, updates profile | Receives online personal training status, confirmation | Primary domain user |
| 2 | Online Personal Training Administrator | Primary | Configures online personal training settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Online Personal Training System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Huấn Luyện Viên Cá Nhân Trực Tuyến. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
