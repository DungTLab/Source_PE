# Context Diagram — Paraolympics & Adaptive Sports System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Paraolympics & Adaptive Sports User"]
    DomainAdmin["Paraolympics & Adaptive Sports Administrator"]
    ...
    System(("Paraolympics & Adaptive Sports System"))

    PrimaryUser -->|"Submits paraolympics & adaptive sports requests"| System
    System -->|"Receives paraolympics & adaptive sports status"| PrimaryUser
    DomainAdmin -->|"Configures paraolympics & adaptive sports settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Paraolympics & Adaptive Sports User | Primary | Submits paraolympics & adaptive sports requests, updates profile | Receives paraolympics & adaptive sports status, confirmation | Primary domain user |
| 2 | Paraolympics & Adaptive Sports Administrator | Primary | Configures paraolympics & adaptive sports settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Paraolympics & Adaptive Sports System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Thể Thao Người Khuyết Tật (Paraolympics). The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
