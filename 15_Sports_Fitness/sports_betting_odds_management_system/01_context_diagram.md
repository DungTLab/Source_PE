# Context Diagram — Sports Betting & Odds Management System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Sports Betting & Odds User"]
    DomainAdmin["Sports Betting & Odds Administrator"]
    ...
    System(("Sports Betting & Odds Management System"))

    PrimaryUser -->|"Submits sports betting & odds requests"| System
    System -->|"Receives sports betting & odds status"| PrimaryUser
    DomainAdmin -->|"Configures sports betting & odds settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Sports Betting & Odds User | Primary | Submits sports betting & odds requests, updates profile | Receives sports betting & odds status, confirmation | Primary domain user |
| 2 | Sports Betting & Odds Administrator | Primary | Configures sports betting & odds settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Sports Betting & Odds Management System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Quản Lý Cá Cược và Tỷ Lệ Cược Thể Thao. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
