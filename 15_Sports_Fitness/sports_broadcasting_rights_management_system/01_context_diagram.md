# Context Diagram — Sports Broadcasting Rights Management System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Sports Broadcasting Rights User"]
    DomainAdmin["Sports Broadcasting Rights Administrator"]
    ...
    System(("Sports Broadcasting Rights Management System"))

    PrimaryUser -->|"Submits sports broadcasting rights requests"| System
    System -->|"Receives sports broadcasting rights status"| PrimaryUser
    DomainAdmin -->|"Configures sports broadcasting rights settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Sports Broadcasting Rights User | Primary | Submits sports broadcasting rights requests, updates profile | Receives sports broadcasting rights status, confirmation | Primary domain user |
| 2 | Sports Broadcasting Rights Administrator | Primary | Configures sports broadcasting rights settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Sports Broadcasting Rights Management System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Quản Lý Bản Quyền Truyền Hình Thể Thao. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
