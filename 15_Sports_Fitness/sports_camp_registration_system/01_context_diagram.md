# Context Diagram — Sports Camp Registration System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Sports Camp Registration User"]
    DomainAdmin["Sports Camp Registration Administrator"]
    ...
    System(("Sports Camp Registration System"))

    PrimaryUser -->|"Submits sports camp registration requests"| System
    System -->|"Receives sports camp registration status"| PrimaryUser
    DomainAdmin -->|"Configures sports camp registration settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Sports Camp Registration User | Primary | Submits sports camp registration requests, updates profile | Receives sports camp registration status, confirmation | Primary domain user |
| 2 | Sports Camp Registration Administrator | Primary | Configures sports camp registration settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Sports Camp Registration System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Đăng Ký Trại Hè Thể Thao. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
