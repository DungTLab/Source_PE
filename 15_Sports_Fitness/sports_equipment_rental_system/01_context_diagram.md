# Context Diagram — Sports Equipment Rental System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Sports Equipment Rental User"]
    DomainAdmin["Sports Equipment Rental Administrator"]
    ...
    System(("Sports Equipment Rental System"))

    PrimaryUser -->|"Submits sports equipment rental requests"| System
    System -->|"Receives sports equipment rental status"| PrimaryUser
    DomainAdmin -->|"Configures sports equipment rental settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Sports Equipment Rental User | Primary | Submits sports equipment rental requests, updates profile | Receives sports equipment rental status, confirmation | Primary domain user |
| 2 | Sports Equipment Rental Administrator | Primary | Configures sports equipment rental settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Sports Equipment Rental System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Cho Thuê Dụng Cụ Thể Thao. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
