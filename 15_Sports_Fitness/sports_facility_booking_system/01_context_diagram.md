# Context Diagram — Sports Facility Booking System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Sports Facility Booking User"]
    DomainAdmin["Sports Facility Booking Administrator"]
    ...
    PrimaryUser -->|"Submits sports facility booking requests"| System
    System -->|"Receives sports facility booking status"| PrimaryUser
    DomainAdmin -->|"Configures sports facility booking settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Sports Facility Booking User | Primary | Submits sports facility booking requests, updates profile | Receives sports facility booking status, confirmation | Primary domain user |
| 2 | Sports Facility Booking Administrator | Primary | Configures sports facility booking settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Sports Facility Booking System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Đặt Sân và Cơ Sở Vật Chất Thể Thao. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
