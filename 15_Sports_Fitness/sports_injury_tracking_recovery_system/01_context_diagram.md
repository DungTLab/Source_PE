# Context Diagram — Sports Injury Tracking & Recovery System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Sports Injury Tracking & Recovery User"]
    DomainAdmin["Sports Injury Tracking & Recovery Administrator"]
    ...
    System(("Sports Injury Tracking & Recovery System"))

    PrimaryUser -->|"Submits sports injury tracking & recovery requests"| System
    System -->|"Receives sports injury tracking & recovery status"| PrimaryUser
    DomainAdmin -->|"Configures sports injury tracking & recovery settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Sports Injury Tracking & Recovery User | Primary | Submits sports injury tracking & recovery requests, updates profile | Receives sports injury tracking & recovery status, confirmation | Primary domain user |
| 2 | Sports Injury Tracking & Recovery Administrator | Primary | Configures sports injury tracking & recovery settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Sports Injury Tracking & Recovery System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Theo Dõi và Phục Hồi Chấn Thương Thể Thao. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
