# Context Diagram — Sports Nutrition Management System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Sports Nutrition User"]
    DomainAdmin["Sports Nutrition Administrator"]
    ...
    System(("Sports Nutrition Management System"))

    PrimaryUser -->|"Submits sports nutrition requests"| System
    System -->|"Receives sports nutrition status"| PrimaryUser
    DomainAdmin -->|"Configures sports nutrition settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Sports Nutrition User | Primary | Submits sports nutrition requests, updates profile | Receives sports nutrition status, confirmation | Primary domain user |
| 2 | Sports Nutrition Administrator | Primary | Configures sports nutrition settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Sports Nutrition Management System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Quản Lý Dinh Dưỡng Thể Thao. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
