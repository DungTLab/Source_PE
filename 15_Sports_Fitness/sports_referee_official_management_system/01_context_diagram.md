# Context Diagram — Sports Referee & Official Management System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Sports Referee & Official User"]
    DomainAdmin["Sports Referee & Official Administrator"]
    ...
    System(("Sports Referee & Official Management System"))

    PrimaryUser -->|"Submits sports referee & official requests"| System
    System -->|"Receives sports referee & official status"| PrimaryUser
    DomainAdmin -->|"Configures sports referee & official settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Sports Referee & Official User | Primary | Submits sports referee & official requests, updates profile | Receives sports referee & official status, confirmation | Primary domain user |
| 2 | Sports Referee & Official Administrator | Primary | Configures sports referee & official settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Sports Referee & Official Management System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Quản Lý Trọng Tài và Điều Hành Thể Thao. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
