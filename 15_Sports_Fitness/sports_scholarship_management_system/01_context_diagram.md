# Context Diagram — Sports Scholarship Management System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Sports Scholarship User"]
    DomainAdmin["Sports Scholarship Administrator"]
    ...
    System(("Sports Scholarship Management System"))

    PrimaryUser -->|"Submits sports scholarship requests"| System
    System -->|"Receives sports scholarship status"| PrimaryUser
    DomainAdmin -->|"Configures sports scholarship settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Sports Scholarship User | Primary | Submits sports scholarship requests, updates profile | Receives sports scholarship status, confirmation | Primary domain user |
| 2 | Sports Scholarship Administrator | Primary | Configures sports scholarship settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Sports Scholarship Management System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Quản Lý Học Bổng Thể Thao. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
