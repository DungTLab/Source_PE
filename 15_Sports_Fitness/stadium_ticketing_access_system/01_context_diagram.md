# Context Diagram — Stadium Ticketing & Access System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Stadium Ticketing & Access User"]
    DomainAdmin["Stadium Ticketing & Access Administrator"]
    ...
    System(("Stadium Ticketing & Access System"))

    PrimaryUser -->|"Submits stadium ticketing & access requests"| System
    System -->|"Receives stadium ticketing & access status"| PrimaryUser
    DomainAdmin -->|"Configures stadium ticketing & access settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Stadium Ticketing & Access User | Primary | Submits stadium ticketing & access requests, updates profile | Receives stadium ticketing & access status, confirmation | Primary domain user |
| 2 | Stadium Ticketing & Access Administrator | Primary | Configures stadium ticketing & access settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Stadium Ticketing & Access System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Bán Vé và Kiểm Soát Ra Vào Sân Vận Động. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
