# Conceptual ERD — Swimming Pool Management System
...
erDiagram
    ...
    SWIMMING_POOL_USER ||--o{ SWIMMING_POOL_EVENT : "contains"
    SWIMMING_POOL_USER ||--o{ TRANSACTION_LOG : "generates"
    SWIMMING_POOL_EVENT ||--o{ RESOURCE_ITEM : "allocates"
...
|---|-------------|-----------------|-------------|----------------|--------------------|
| 1 | SWIMMING_POOL_USER | Người dùng chính | Stores user profiles and login details for Swimming Pool Management System | user_id PK, full_name string, email string | places REQUEST, receives NOTIFICATION |
| 2 | SWIMMING_POOL_EVENT | Sự kiện / Giải đấu | Tracks scheduled events, matches, or sessions in Swimming Pool Management System | event_id PK, title string, event_date datetime | has many SESSION, manages RESOURCE |
...
|---|-------------|-------------|-----------|-------------------|----------------------|
| 1 | SWIMMING_POOL_USER | one-to-many | SWIMMING_POOL_EVENT | contains | Single entity parent relates to multiple child records |
| 2 | SWIMMING_POOL_USER | one-to-many | TRANSACTION_LOG | generates | Creates financial logs for processing |
| 3 | SWIMMING_POOL_EVENT | one-to-many | RESOURCE_ITEM | allocates | Assigns physical resources for events |
