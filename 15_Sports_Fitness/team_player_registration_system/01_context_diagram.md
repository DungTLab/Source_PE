# Context Diagram — Team & Player Registration System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Team & Player Registration User"]
    DomainAdmin["Team & Player Registration Administrator"]
    ...
    System(("Team & Player Registration System"))

    PrimaryUser -->|"Submits team & player registration requests"| System
    System -->|"Receives team & player registration status"| PrimaryUser
    DomainAdmin -->|"Configures team & player registration settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Team & Player Registration User | Primary | Submits team & player registration requests, updates profile | Receives team & player registration status, confirmation | Primary domain user |
| 2 | Team & Player Registration Administrator | Primary | Configures team & player registration settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Team & Player Registration System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Đăng Ký Đội và Vận Động Viên. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
