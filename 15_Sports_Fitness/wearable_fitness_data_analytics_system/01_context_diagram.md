# Context Diagram — Wearable Fitness Data Analytics System

## Mermaid Code

```mermaid
flowchart LR
    PrimaryUser["Wearable Fitness Data Analytics User"]
    DomainAdmin["Wearable Fitness Data Analytics Administrator"]
    ...
    System(("Wearable Fitness Data Analytics System"))

    PrimaryUser -->|"Submits wearable fitness data analytics requests"| System
    System -->|"Receives wearable fitness data analytics status"| PrimaryUser
    DomainAdmin -->|"Configures wearable fitness data analytics settings"| System
...
|---|-------|------------|---------------------|---------------------------|-------|
| 1 | Wearable Fitness Data Analytics User | Primary | Submits wearable fitness data analytics requests, updates profile | Receives wearable fitness data analytics status, confirmation | Primary domain user |
| 2 | Wearable Fitness Data Analytics Administrator | Primary | Configures wearable fitness data analytics settings, manages permissions | Receives audit logs, system performance metrics | System administrator |
...
The Wearable Fitness Data Analytics System provides an end-to-end digital ecosystem designed specifically to automate and optimize operational workflows for Hệ Thống Phân Tích Dữ Liệu Fitness Từ Thiết Bị Đeo. The system boundary includes core module capabilities such as user registrations, event/session scheduling, data processing, resource allocation, and automated reporting. External integrations with payment gateways, messaging networks, IoT sensor hardware, and regulatory audit portals operate outside the core application server but maintain secure API interactions. Manual paper-based logs and unintegrated external physical facilities remain outside the system boundary. overall system enforces strict role-based access control (RBAC) and encryption to safeguard data privacy.
